...
int main (int argc, char * const * argv)
{
	...
	paper::add_node_options (description);
	...
	if (!paper::handle_node_options (vm))
	{
	}
	...
	{
		paper::inactive_node node (data_path);
	...
	else if (vm.count ("debug_bootstrap_generate"))
	{
		if (vm.count ("key") == 1)
		{
			...
			{
				paper::keypair genesis (key.to_string ());
	...
	else if (vm.count ("debug_dump_representatives"))
	{
		...
		std::map<paper::account, paper::uint128_t> calculated;
	...
	else if (vm.count ("debug_profile_kdf"))
	{
		...
		paper::uint256_union salt (0);
		std::string password ("");
		for (; true;)
		{
			auto begin1 (std::chrono::high_resolution_clock::now ());
			auto success (argon2_hash (1, paper::wallet_store::kdf_work, 1, password.data (), password.size (), salt.bytes.data (), salt.bytes.size (), result.bytes.data (), result.bytes.size (), NULL, 0, Argon2_d, 0x10));
	...
	{
		paper::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
		paper::change_block block (0, 0, paper::keypair ().prv, 0, 0);
			...
			if (!result)
			{
				...
				if (!error)
				{
					...
					if (!error)
					{
						...
						auto opencl (paper::opencl_work::create (true, { platform, device, threads }, logging));
						paper::work_pool work_pool (std::numeric_limits<unsigned>::max (), opencl ? [&opencl](paper::uint256_union const & root_a) {
	...
	else if (vm.count ("debug_verify_profile"))
	{
		...
		{
			paper::validate_message (key.pub, message, signature);
		}
