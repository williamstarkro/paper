#include <paper/paper_node/daemon.hpp>
...
paper_daemon::daemon_config::daemon_config (boost::filesystem::path const & application_path_a) :
rpc_enable (false),
opencl_enable (false)
{
}

void paper_daemon::daemon_config::serialize_json (boost::property_tree::ptree & tree_a)
{
...
bool paper_daemon::daemon_config::deserialize_json (bool & upgraded_a, boost::property_tree::ptree & tree_a)
{
	auto error (false);
	try
	{
...
bool paper_daemon::daemon_config::upgrade_json (unsigned version_a, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	switch (version_a)
	{
...
void paper_daemon::daemon::run (boost::filesystem::path const & data_path)
{
	boost::filesystem::create_directories (data_path);
	paper_daemon::daemon_config config (data_path);
	auto config_path ((data_path / "config.json"));
	std::fstream config_file;
	std::unique_ptr<paper::thread_runner> runner;
	auto error (paper::fetch_object (config, config_path, config_file));
	if (!error)
	{
		config.node.logging.init (data_path);
		config_file.close ();
		boost::asio::io_service service;
		auto opencl (paper::opencl_work::create (config.opencl_enable, config.opencl, config.node.logging));
		paper::work_pool opencl_work (config.node.work_threads, opencl ? [&opencl](paper::uint256_union const & root_a) {
		...
		try
		{
			auto node (std::make_shared<paper::node> (init, service, data_path, alarm, config.node, opencl_work));
			if (!init.error ())
			{
				node->start ();
				std::unique_ptr<paper::rpc> rpc = get_rpc (service, *node, config.rpc);
				if (rpc && config.rpc_enable)
				{
					rpc->start ();
				}
				runner = std::make_unique<paper::thread_runner> (service, node->config.io_threads);
				runner->join ();
			}
			else
			{
