...
TEST (wallet, construction)
{
	...
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
TEST (wallet, status)
{
	...
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet](paper_qt::status_types status_ty) {
	...
	system.nodes[0]->peers.insert (paper::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (paper_qt::status_types::synchronizing));
	auto iterations (0);
	while (!wallet_has (paper_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (paper_qt::status_types::disconnected));
}
...
TEST (wallet, select_account)
{
	...
	paper::system system (24000, 1);
	auto wallet_l (system.nodes[0]->wallets.create (paper::uint256_union ()));
	paper::public_key key1 (wallet_l->deterministic_insert ());
	paper::public_key key2 (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (paper::keypair ().prv);
	{
		paper::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
		account = system.account (transaction, 0);
	}
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		paper::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
...
TEST (wallet, send)
{
	...
	paper::public_key key1 (system.wallet (1)->insert_adhoc (paper::keypair ().prv));
	auto account (paper::test_genesis_key.pub);
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, paper::test_genesis_key.pub.to_account ().c_str ());
...
TEST (wallet, create_open_receive)
{
	...
	QTest::keyClicks (wallet->block_creation.representative, paper::test_genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
...
TEST (history, short_text)
{
	...
	paper_qt::history history (ledger, paper::test_genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	paper::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, paper::test_genesis_key.prv.data.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	paper::raw_key seed3;
	system.wallet (0)->store.seed (seed3, paper::transaction (system.wallet (0)->store.environment, nullptr, false));
	auto wallet_key (key1);
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	paper::raw_key seed1;
	system.wallet (0)->store.seed (seed1, paper::transaction (system.wallet (0)->store.environment, nullptr, false));
	...
	paper::raw_key seed2;
	system.wallet (0)->store.seed (seed2, paper::transaction (system.wallet (0)->store.environment, nullptr, false));
...
TEST (wallet, seed_work_generation)
{
	...
	paper::system system (24000, 1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	auto iterations (0);
	uint64_t work_start;
	system.wallet (0)->store.work_get (paper::transaction (system.wallet (0)->store.environment, nullptr, false), key1, work_start);
	uint64_t work (work_start);
	while (work == work_start)
	{
		system.poll ();
		system.wallet (0)->store.work_get (paper::transaction (system.wallet (0)->store.environment, nullptr, false), key1, work);
		++iterations;
		ASSERT_LT (iterations, 200);
	}
	ASSERT_FALSE (paper::work_validate (system.nodes[0]->ledger.latest_root (paper::transaction (system.wallet (0)->store.environment, nullptr, false), key1), work));
}
...
TEST (wallet, backup_seed)
{
	...
	paper::raw_key seed;
	system.wallet (0)->store.seed (seed, paper::transaction (system.wallet (0)->store.environment, nullptr, false));
...
TEST (wallet, import_locked)
{
	...
	paper::system system (24000, 1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	system.wallet (0)->store.rekey (paper::transaction (system.wallet (0)->store.environment, nullptr, true), "1");
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
...
TEST (wallet, synchronizing)
{
	...
	auto wallet (std::make_shared<paper_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
		paper::transaction transaction (system1.nodes[0]->store.environment, nullptr, true);
		auto latest (system1.nodes[0]->ledger.latest (transaction, paper::genesis_account));
		paper::send_block send (latest, key1, 0, paper::test_genesis_key.prv, paper::test_genesis_key.pub, system1.work.generate (latest));
		system1.nodes[0]->ledger.process (transaction, send);
	}
	ASSERT_EQ (0, wallet->active_status.active.count (paper_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	auto iterations0 (0);
	while (wallet->active_status.active.count (paper_qt::status_types::synchronizing) == 0)
	{
		system0.poll ();
		system1.poll ();
		test_application->processEvents ();
	...
	auto iterations1 (0);
	while (wallet->active_status.active.count (paper_qt::status_types::synchronizing) == 1)
	{
		system0.poll ();
		system1.poll ();
		test_application->processEvents ();
