...
bool update_config (qt_wallet_config & config_a, boost::filesystem::path const & config_path_a, std::fstream & config_file_a)
{
	...
	auto error (false);
	if (!paper::fetch_object (config_a, config_path_a, config_file_a))
	{
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path)
{
	...
	if (!error)
	{
		...
		std::shared_ptr<paper_qt::wallet> gui;
		...
		paper::work_pool work (config.node.work_threads, opencl ? [&opencl](paper::uint256_union const & root_a) {
		...
		node = std::make_shared<paper::node> (init, service, data_path, alarm, config.node, work);
		if (!init.error ())
		{
			...
			if (config.account.is_zero () || !wallet->exists (config.account))
			{
				...
				{
					paper::uint256_union account (existing->first.uint256 ());
			...
			paper::thread_runner runner (service, node->config.io_threads);
			QObject::connect (&application, &QApplication::aboutToQuit, [&]() {
				rpc->stop ();
				node->stop ();
			});
			application.postEvent (&processor, new paper_qt::eventloop_event ([&]() {
				gui = std::make_shared<paper_qt::wallet> (application, processor, *node, wallet, config.account);
				splash->close ();
				gui->start ();
