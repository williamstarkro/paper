...
__kernel void paper_work (__global ulong * attempt, __global ulong * result_a, __global uchar * item_a)
{
...
paper::opencl_environment::opencl_environment (bool & error_a)
{
	...
	{
		paper::opencl_platform platform;
		platform.platform = *i;
...
void paper::opencl_environment::dump (std::ostream & stream)
{
	auto index (0);
	auto device_count (0);
...
paper::opencl_config::opencl_config () :
platform (0),
device (0),
threads (1024 * 1024)
{
...
paper::opencl_config::opencl_config (unsigned platform_a, unsigned device_a, unsigned threads_a) :
platform (platform_a),
device (device_a),
threads (threads_a)
{
...
void paper::opencl_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool paper::opencl_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto result (false);
	try
	{
...
paper::opencl_work::opencl_work (bool & error_a, paper::opencl_config const & config_a, paper::opencl_environment & environment_a, paper::logging & logging_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logging (logging_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			paper::random_pool.GenerateBlock (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							size_t item_size (sizeof (paper::uint256_union));
							...
							if (!error_a)
							{
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										kernel = clCreateKernel (program, "paper_work", &kernel_error);
...
boost::optional<uint64_t> paper::opencl_work::generate_work (paper::uint256_union const & root_a)
{
	...
	while (paper::work_validate (root_a, result) && !error)
	{
		result = rand.next ();
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (paper::uint256_union), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<paper::opencl_work> paper::opencl_work::create (bool create_a, paper::opencl_config const & config_a, paper::logging & logging_a)
{
	std::unique_ptr<paper::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new paper::opencl_work (error, config_a, environment, logging_a));
			if (error)
			{
				result.reset ();
			}
