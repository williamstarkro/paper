...
#ifdef PAPER_SECURE_RPC
#include <paper/node/rpc_secure.hpp>
#endif

paper::rpc_secure_config::rpc_secure_config () :
enable (false),
verbose_logging (false)
{
}

void paper::rpc_secure_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool paper::rpc_secure_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto error (false);
	try
	{
...
paper::rpc::rpc (boost::asio::io_service & service_a, paper::node & node_a, paper::rpc_config const & config_a) :
acceptor (service_a),
config (config_a),
node (node_a)
{
...
{
	auto endpoint (paper::tcp_endpoint (config.address, config.port));
...
void paper::rpc::accept ()
{
	auto connection (std::make_shared<paper::rpc_connection> (node, *this));
...
paper::rpc_handler::rpc_handler (paper::node & node_a, paper::rpc & rpc_a, std::string const & body_a, std::function<void(boost::property_tree::ptree const &)> const & response_a) :
body (body_a),
node (node_a),
rpc (rpc_a),
response (response_a)
...
void paper::rpc::observer_action (paper::account const & account_a)
{
	std::shared_ptr<paper::payment_observer> observer;
	{
...
void paper::error_response (std::function<void(boost::property_tree::ptree const &)> response_a, std::string const & message_a)
{
...
void paper::rpc_handler::account_block_count ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::uint256_union account;
	auto error (account.decode_account (account_text));
...
void paper::rpc_handler::account_create ()
{
	if (rpc.config.enable_control)
	{
		...
		if (!error)
		{
			...
			if (existing != node.wallets.items.end ())
			{
				...
				paper::account new_key (existing->second->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree response_l;
...
void paper::rpc_handler::account_get ()
{
...
void paper::rpc_handler::account_info ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::uint256_union account;
	auto error (account.decode_account (account_text));
...
void paper::rpc_handler::account_key ()
{
	std::string account_text (request.get<std::string> ("account"));
...
void paper::rpc_handler::account_list ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			for (auto i (existing->second->store.begin (transaction)), j (existing->second->store.end ()); i != j; ++i)
			{
				...
				entry.put ("", paper::uint256_union (i->first.uint256 ()).to_account ());
...
void paper::rpc_handler::account_move ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::account_remove ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::account_representative ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::account account;
	auto error (account.decode_account (account_text));
...
void paper::rpc_handler::account_representative_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (!error)
					{
						...
						{
							auto work_error (paper::from_string_hex (work_text.get (), work));
							if (work_error)
							{
						...
						if (work)
						{
							...
							if (!node.store.account_get (transaction, account, info))
							{
								if (!paper::work_validate (info.head, work))
								{
						...
						wallet->change_async (account, representative, [response_a](std::shared_ptr<paper::block> block) {
...
void paper::rpc_handler::account_weight ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::uint256_union account;
	auto error (account.decode_account (account_text));
...
void paper::rpc_handler::accounts_create ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::accounts_frontiers ()
{
	boost::property_tree::ptree response_l;
...
void paper::rpc_handler::accounts_pending ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	paper::uint128_union threshold (0);
	bool source (false);
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!account.decode_account (account_text))
		{
			...
			paper::account end (account.number () + 1);
			for (auto i (node.store.pending_begin (transaction, paper::pending_key (account, 0))), n (node.store.pending_begin (transaction, paper::pending_key (end, 0))); i != n && peers_l.size () < count; ++i)
			{
				...
				{
					paper::pending_info info (i->second);
...
void paper::rpc_handler::blocks_info ()
{
	bool pending (false);
	boost::optional<bool> pending_optional (request.get_optional<bool> ("pending"));
	if (pending_optional.is_initialized ())
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		...
		if (!error)
		{
			...
			if (block != nullptr)
			{
				...
				if (pending)
				{
					auto block_l (dynamic_cast<paper::send_block *> (block.get ()));
					bool exists (false);
					if (block_l != nullptr)
					{
						auto destination (block_l->hashables.destination);
						exists = node.store.pending_exists (transaction, paper::pending_key (destination, hash));
					}
					entry.put ("pending", exists ? "1" : "0");
				}
				if (source)
				{
					paper::block_hash source_hash (block->source ());
					std::unique_ptr<paper::block> block_a (node.store.block_get (transaction, source_hash));
					if (block_a != nullptr)
					{
...
void paper::rpc_handler::block_account ()
{
	std::string hash_text (request.get<std::string> ("hash"));
...
void paper::rpc_handler::block_count ()
{
	paper::transaction transaction (node.store.environment, nullptr, false);
	boost::property_tree::ptree response_l;
...
void paper::rpc_handler::block_count_type ()
{
	paper::transaction transaction (node.store.environment, nullptr, false);
	paper::block_counts count (node.store.block_count (transaction));
	boost::property_tree::ptree response_l;
...
void paper::rpc_handler::block_create ()
{
	if (rpc.config.enable_control)
	{
		std::string type (request.get<std::string> ("type"));
		paper::uint256_union wallet (0);
		...
		paper::uint256_union representative (0);
		...
		paper::uint256_union destination (0);
		...
		if (prv.data != 0)
		{
			...
			if (type == "open")
			{
				if (representative != 0 && source != 0)
				{
					...
					paper::open_block open (source, representative, pub, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "receive")
			{
				if (source != 0 && previous != 0)
				{
					...
					paper::receive_block receive (previous, source, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "change")
			{
				if (representative != 0 && previous != 0)
				{
					...
					paper::change_block change (previous, representative, prv, pub, work);
					boost::property_tree::ptree response_l;
...
void paper::rpc_handler::successors ()
{
...
void paper::rpc_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!paper::parse_port (port_text, port))
		{
			node.bootstrap_initiator.bootstrap (paper::endpoint (address, port));
			boost::property_tree::ptree response_l;
...
void paper::rpc_handler::bootstrap_any ()
{
...
void paper::rpc_handler::delegators ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::account account;
	auto error (account.decode_account (account_text));
...
void paper::rpc_handler::delegators_count ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::account account;
	auto error (account.decode_account (account_text));
...
void paper::rpc_handler::deterministic_key ()
{
...
void paper::rpc_handler::frontiers ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string count_text (request.get<std::string> ("count"));
	paper::account start;
	if (!start.decode_account (account_text))
	{
		...
		if (!decode_unsigned (count_text, count))
		{
			...
			{
				frontiers.put (paper::account (i->first.uint256 ()).to_account (), paper::account_info (i->second).head.to_string ());
			}
			response_l.add_child ("frontiers", frontiers);
			response (response_l);
		}
...
void paper::rpc_handler::frontier_count ()
{
	paper::transaction transaction (node.store.environment, nullptr, false);
...
namespace
{
class history_visitor : public paper::block_visitor
{
public:
	history_visitor (paper::rpc_handler & handler_a, paper::transaction & transaction_a, boost::property_tree::ptree & tree_a, paper::block_hash const & hash_a) :
	handler (handler_a),
	...
	void open_block (paper::open_block const & block_a)
	{
		...
		{
			tree.put ("account", paper::genesis_account.to_account ());
			tree.put ("amount", paper::genesis_amount.convert_to<std::string> ());
		}
	}
	...
	paper::rpc_handler & handler;
	paper::transaction & transaction;
	boost::property_tree::ptree & tree;
	paper::block_hash const & hash;
};
}

void paper::rpc_handler::history ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	std::string count_text (request.get<std::string> ("count"));
	paper::block_hash hash;
...
void paper::rpc_handler::account_history ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string count_text (request.get<std::string> ("count"));
	paper::uint256_union account;
...
void paper::rpc_handler::keepalive ()
{
	if (rpc.config.enable_control)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void paper::rpc_handler::key_create ()
{
	boost::property_tree::ptree response_l;
	paper::keypair pair;
...
void paper::rpc_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	paper::uint256_union prv;
...
void paper::rpc_handler::ledger ()
{
	if (rpc.config.enable_control)
	{
		paper::account start (0);
		uint64_t count (std::numeric_limits<uint64_t>::max ());
		bool sorting (false);
		boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
		if (account_text.is_initialized ())
		...
		else // Sorting
		{
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				paper::account account (i->second);
				response_l.put ("frontier", info.head.to_string ());
				response_l.put ("open_block", info.open_block.to_string ());
				response_l.put ("representative_block", info.rep_block.to_string ());
				std::string assetKey;
...
void paper::rpc_handler::mpaper_from_raw ()
{
...
void paper::rpc_handler::mpaper_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	paper::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void paper::rpc_handler::kpaper_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	paper::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void paper::rpc_handler::kpaper_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	paper::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void paper::rpc_handler::password_change ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::password_enter ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void paper::rpc_handler::password_valid (bool wallet_locked = false)
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void paper::rpc_handler::pending ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::account account;
	if (!account.decode_account (account_text))
...
void paper::rpc_handler::pending_exists ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	paper::uint256_union hash;
	auto error (hash.decode_hex (hash_text));
	if (!error)
	{
		...
		if (block != nullptr)
		{
			...
			if (block_l != nullptr)
			{
				...
				exists = node.store.pending_exists (transaction, paper::pending_key (account, hash));
			}
			boost::property_tree::ptree response_l;
...
void paper::rpc_handler::payment_begin ()
{
	...
	if (!id.decode_hex (id_text))
	{
		...
		{
			paper::transaction transaction (node.store.environment, nullptr, true);
			std::shared_ptr<paper::wallet> wallet (existing->second);
...
void paper::rpc_handler::payment_init ()
{
	std::string id_text (request.get<std::string> ("wallet"));
	paper::uint256_union id;
	if (!id.decode_hex (id_text))
...
void paper::rpc_handler::payment_end ()
{
	std::string id_text (request.get<std::string> ("wallet"));
	std::string account_text (request.get<std::string> ("account"));
	paper::uint256_union id;
...
void paper::rpc_handler::payment_wait ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string amount_text (request.get<std::string> ("amount"));
	...
	if (!account.decode_account (account_text))
	{
		...
		if (!amount.decode_dec (amount_text))
		{
			...
			{
				{
					auto observer (std::make_shared<paper::payment_observer> (response, rpc, account, amount));
...
void paper::rpc_handler::process ()
{
	std::string block_text (request.get<std::string> ("block"));
	...
	if (block != nullptr)
	{
		if (!paper::work_validate (*block))
		{
			auto hash (block->hash ());
			node.block_arrival.add (hash);
			paper::process_return result;
			std::shared_ptr<paper::block> block_a (std::move (block));
			{
				paper::transaction transaction (node.store.environment, nullptr, true);
...
void paper::rpc_handler::paper_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	paper::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void paper::rpc_handler::paper_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	paper::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void paper::rpc_handler::receive ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						if (!error)
						{
							...
							if (block != nullptr)
							{
								if (node.store.pending_exists (transaction, paper::pending_key (account, hash)))
								{
									uint64_t work (0);
									boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
									if (work_text.is_initialized ())
									...
									if (work)
									{
										...
										if (!paper::work_validate (head, work))
										{
											paper::transaction transaction_a (node.store.environment, nullptr, true);
									...
									auto response_a (response);
									existing->second->receive_async (std::move (block), account, paper::genesis_amount, [response_a](std::shared_ptr<paper::block> block_a) {
										paper::uint256_union hash_a (0);
										if (block_a != nullptr)
										{
											hash_a = block_a->hash ();
										}
...
void paper::rpc_handler::receive_minimum ()
{
	if (rpc.config.enable_control)
	{
		boost::property_tree::ptree response_l;
...
void paper::rpc_handler::receive_minimum_set ()
{
	if (rpc.config.enable_control)
	{
		std::string amount_text (request.get<std::string> ("amount"));
...
void paper::rpc_handler::representatives ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	bool sorting (false);
	boost::optional<std::string> count_text (request.get_optional<std::string> ("count"));
	...
	{
		std::vector<std::pair<paper::uint128_union, std::string>> representation;
...
void paper::rpc_handler::republish ()
{
	...
	if (!error)
	{
		...
		if (block != nullptr)
		{
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				...
				{
					paper::block_hash source (block->source ());
					std::unique_ptr<paper::block> block_a (node.store.block_get (transaction, source));
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					auto block_s (dynamic_cast<paper::send_block *> (block_b.get ()));
					if (block_s != nullptr)
					{
						auto destination (block_s->hashables.destination);
						auto exists (node.store.pending_exists (transaction, paper::pending_key (destination, hash)));
						if (!exists)
						{
							paper::block_hash previous (node.ledger.latest (transaction, destination));
							std::unique_ptr<paper::block> block_d (node.store.block_get (transaction, previous));
...
void paper::rpc_handler::search_pending ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::search_pending_all ()
{
	if (rpc.config.enable_control)
	{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							if (assetKey >= amount.number ())
							{
								...
								auto response_a (response);
								existing->second->send_async (source, destination, amount.number (), [response_a](std::shared_ptr<paper::block> block_a) {
									if (block_a != nullptr)
									{
										paper::uint256_union hash (block_a->hash ());
										boost::property_tree::ptree response_l;
										response_l.put ("block", hash.to_string ());
										response_a (response_l);
									}
...
void paper::rpc_handler::unchecked ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	boost::optional<std::string> count_text (request.get_optional<std::string> ("count"));
	if (count_text.is_initialized ())
...
void paper::rpc_handler::unchecked_clear ()
{
	if (rpc.config.enable_control)
	{
		paper::transaction transaction (node.store.environment, nullptr, true);
...
void paper::rpc_handler::unchecked_get ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	paper::uint256_union hash;
	auto error (hash.decode_hex (hash_text));
...
void paper::rpc_handler::unchecked_keys ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	...
	for (auto i (node.store.unchecked_begin (transaction, key)), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
	{
		...
		std::string contents;
		block->serialize_json (contents);
		entry.put ("key", paper::block_hash (i->first.uint256 ()).to_string ());
...
void paper::rpc_handler::version ()
{
	boost::property_tree::ptree response_l;
	response_l.put ("rpc_version", "1");
	response_l.put ("store_version", std::to_string (node.store_version ()));
	response_l.put ("node_vendor", boost::str (boost::format ("Paper %1%.%2%") % PAPER_VERSION_MAJOR % PAPER_VERSION_MINOR));
	response (response_l);
}

void paper::rpc_handler::validate_account_number ()
{
	std::string account_text (request.get<std::string> ("account"));
	paper::uint256_union account;
	auto error (account.decode_account (account_text));
...
void paper::rpc_handler::wallet_add ()
{
	if (rpc.config.enable_control)
	{
		std::string key_text (request.get<std::string> ("key"));
...
void paper::rpc_handler::wallet_change_seed ()
{
	if (rpc.config.enable_control)
	{
		std::string seed_text (request.get<std::string> ("seed"));
...
void paper::rpc_handler::wallet_contains ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union account;
...
void paper::rpc_handler::wallet_create ()
{
	if (rpc.config.enable_control)
	{
...
void paper::rpc_handler::wallet_destroy ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::wallet_export ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void paper::rpc_handler::wallet_frontiers ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void paper::rpc_handler::wallet_key_valid ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void paper::rpc_handler::wallet_lock ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::wallet_pending ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
			{
				...
				boost::property_tree::ptree peers_l;
				paper::account end (account.number () + 1);
				for (auto ii (node.store.pending_begin (transaction, paper::pending_key (account, 0))), nn (node.store.pending_begin (transaction, paper::pending_key (end, 0))); ii != nn && peers_l.size () < count; ++ii)
				{
					paper::pending_key key (ii->first);
					if (threshold.is_zero () && !source)
					{
						boost::property_tree::ptree entry;
						entry.put ("", key.hash.to_string ());
					...
					{
						paper::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source)
							{
...
void paper::rpc_handler::wallet_representative ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	paper::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void paper::rpc_handler::wallet_representative_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::wallet_republish ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void paper::rpc_handler::wallet_work_get ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
		...
		if (!error)
		{
			...
			if (existing != node.wallets.items.end ())
			{
				...
				for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
				{
					...
					works.put (account.to_account (), paper::to_string_hex (work));
				}
...
void paper::rpc_handler::work_generate ()
{
	if (rpc.config.enable_control)
	{
		std::string hash_text (request.get<std::string> ("hash"));
		...
		if (!error)
		{
			...
			node.work.generate (hash, [rpc_l](boost::optional<uint64_t> const & work_a) {
				...
				{
					boost::property_tree::ptree response_l;
					response_l.put ("work", paper::to_string_hex (work_a.value ()));
...
void paper::rpc_handler::work_cancel ()
{
	if (rpc.config.enable_control)
	{
		std::string hash_text (request.get<std::string> ("hash"));
...
void paper::rpc_handler::work_get ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						auto error_work (existing->second->store.work_get (transaction, account, work));
						boost::property_tree::ptree response_l;
						response_l.put ("work", paper::to_string_hex (work));
						response (response_l);
					}
					else
					{
...
void paper::rpc_handler::work_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						uint64_t work;
						auto work_error (paper::from_string_hex (work_text, work));
						if (!work_error)
						{
							existing->second->store.work_put (transaction, account, work);
							boost::property_tree::ptree response_l;
...
void paper::rpc_handler::work_validate ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	paper::block_hash hash;
	auto error (hash.decode_hex (hash_text));
	if (!error)
	{
		...
		if (!work_error)
		{
			auto validate (paper::work_validate (hash, work));
			boost::property_tree::ptree response_l;
...
void paper::rpc_handler::work_peer_add ()
{
	if (rpc.config.enable_control)
	{
		std::string address_text = request.get<std::string> ("address");
...
void paper::rpc_handler::work_peers ()
{
	if (rpc.config.enable_control)
	{
...
void paper::rpc_handler::work_peers_clear ()
{
	if (rpc.config.enable_control)
	{
...
paper::rpc_connection::rpc_connection (paper::node & node_a, paper::rpc & rpc_a) :
node (node_a.shared ()),
rpc (rpc_a),
...
void paper::rpc_connection::parse_connection ()
{
	read ();
}

void paper::rpc_connection::write_result (std::string body, unsigned version)
{
...
void paper::rpc_connection::read ()
{
	...
	boost::beast::http::async_read (socket, buffer, request, [this_l](boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			this_l->node->background ([this_l]() {
				...
				{
					auto handler (std::make_shared<paper::rpc_handler> (*this_l->node, this_l->rpc, this_l->request.body (), response_handler));
...
void paper::rpc_handler::process_request ()
{
	try
	{
		...
		else if (action == "kpaper_from_raw")
		{
			kpaper_from_raw ();
		}
		else if (action == "kpaper_to_raw")
		{
			kpaper_to_raw ();
		}
		...
		else if (action == "mpaper_from_raw")
		{
			mpaper_from_raw ();
		}
		else if (action == "mpaper_to_raw")
		{
			mpaper_to_raw ();
		}
		else if (action == "password_change")
		...
		else if (action == "paper_from_raw")
		{
			paper_from_raw ();
		}
		else if (action == "paper_to_raw")
		{
			paper_to_raw ();
		}
...
void paper::payment_observer::start (uint64_t timeout)
{
	auto this_l (shared_from_this ());
	rpc.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::milliseconds (timeout), [this_l]() {
		this_l->complete (paper::payment_status::nothing);
	});
}

paper::payment_observer::~payment_observer ()
{
}
...
void paper::payment_observer::complete (paper::payment_status status)
{
	...
	if (!already)
	{
		...
		switch (status)
		{
			case paper::payment_status::nothing:
			{
				boost::property_tree::ptree response_l;
			...
			case paper::payment_status::success:
			{
				boost::property_tree::ptree response_l;
...
std::unique_ptr<paper::rpc> paper::get_rpc (boost::asio::io_service & service_a, paper::node & node_a, paper::rpc_config const & config_a)
{
