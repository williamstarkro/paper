...
paper::block_synchronization::block_synchronization (boost::log::sources::logger_mt & log_a) :
log (log_a)
{
}
...
{
public:
	add_dependency_visitor (MDB_txn * transaction_a, paper::block_synchronization & sync_a) :
...
bool paper::block_synchronization::add_dependency (MDB_txn * transaction_a, paper::block const & block_a)
{
...
void paper::block_synchronization::fill_dependencies (MDB_txn * transaction_a)
{
	auto done (false);
	while (!done)
	{
...
paper::sync_result paper::block_synchronization::synchronize_one (MDB_txn * transaction_a)
{
	...
	paper::sync_result result (paper::sync_result::success);
	auto hash (blocks.back ());
	blocks.pop_back ();
	auto block (retrieve (transaction_a, hash));
	if (block != nullptr)
...
paper::sync_result paper::block_synchronization::synchronize (MDB_txn * transaction_a, paper::block_hash const & hash_a)
{
	auto result (paper::sync_result::success);
	blocks.clear ();
	blocks.push_back (hash_a);
	auto cutoff (std::chrono::steady_clock::now () + paper::transaction_timeout);
	while (std::chrono::steady_clock::now () < cutoff && result != paper::sync_result::fork && !blocks.empty ())
	{
...
paper::push_synchronization::push_synchronization (paper::node & node_a, std::function<paper::sync_result (MDB_txn *, paper::block const &)> const & target_a) :
...
bool paper::push_synchronization::synchronized (MDB_txn * transaction_a, paper::block_hash const & hash_a)
{
...
std::unique_ptr<paper::block> paper::push_synchronization::retrieve (MDB_txn * transaction_a, paper::block_hash const & hash_a)
{
	return node.store.block_get (transaction_a, hash_a);
}

paper::sync_result paper::push_synchronization::target (MDB_txn * transaction_a, paper::block const & block_a)
{
	return target_m (transaction_a, block_a);
}

paper::bootstrap_client::bootstrap_client (std::shared_ptr<paper::node> node_a, std::shared_ptr<paper::bootstrap_attempt> attempt_a, paper::tcp_endpoint const & endpoint_a) :
node (node_a),
attempt (attempt_a),
...
void paper::bootstrap_client::start_timeout ()
{
	timeout.expires_from_now (boost::posix_time::seconds (5));
	std::weak_ptr<paper::bootstrap_client> this_w (shared ());
...
void paper::bootstrap_client::stop_timeout ()
{
	size_t killed (timeout.cancel ());
	(void)killed;
}

void paper::bootstrap_client::run ()
{
...
void paper::frontier_req_client::run ()
{
...
std::shared_ptr<paper::bootstrap_client> paper::bootstrap_client::shared ()
{
	return shared_from_this ();
}

paper::frontier_req_client::frontier_req_client (std::shared_ptr<paper::bootstrap_client> connection_a) :
...
void paper::frontier_req_client::receive_frontier ()
{
	auto this_l (shared_from_this ());
	connection->start_timeout ();
	size_t size_l (sizeof (paper::uint256_union) + sizeof (paper::uint256_union));
...
void paper::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		...
		paper::bufferstream account_stream (connection->receive_buffer.data (), sizeof (paper::uint256_union));
		auto error1 (paper::read (account_stream, account));
		assert (!error1);
		paper::block_hash latest;
		paper::bufferstream latest_stream (connection->receive_buffer.data () + sizeof (paper::uint256_union), sizeof (paper::uint256_union));
			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							connection->attempt->add_pull (paper::pull_info (account, latest, info.head));
						}
					}
					next (transaction);
				}
				else
				{
					...
					connection->attempt->add_pull (paper::pull_info (account, latest, paper::block_hash (0)));
				}
			}
			else
			{
...
void paper::frontier_req_client::next (MDB_txn * transaction_a)
{
	auto iterator (connection->node->store.latest_begin (transaction_a, paper::uint256_union (current.number () + 1)));
	if (iterator != connection->node->store.latest_end ())
	{
		current = paper::account (iterator->first.uint256 ());
...
paper::bulk_pull_client::bulk_pull_client (std::shared_ptr<paper::bootstrap_client> connection_a) :
connection (connection_a)
{
...
void paper::bulk_pull_client::request (paper::pull_info const & pull_a)
{
	pull = pull_a;
...
{
	auto this_l (shared_from_this ());
	paper::block_type type (static_cast<paper::block_type> (connection->receive_buffer[0]));
	switch (type)
	{
		case paper::block_type::send:
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, paper::send_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, paper::receive_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, paper::open_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, paper::change_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
...
void paper::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		paper::bufferstream stream (connection->receive_buffer.data (), 1 + size_a);
		std::shared_ptr<paper::block> block (paper::deserialize_block (stream));
		if (block != nullptr && !paper::work_validate (*block))
		{
			...
			connection->attempt->node->block_processor.add (paper::block_processor_item (block));
...
paper::bulk_push_client::bulk_push_client (std::shared_ptr<paper::bootstrap_client> const & connection_a) :
connection (connection_a),
synchronization (*connection->node, [this](MDB_txn * transaction_a, paper::block const & block_a) {
	push_block (block_a);
	return paper::sync_result::success;
})
{
}
...
void paper::bulk_push_client::start ()
{
	...
	boost::asio::async_write (connection->socket, boost::asio::buffer (buffer->data (), buffer->size ()), [this_l, buffer](boost::system::error_code const & ec, size_t size_a) {
		this_l->connection->stop_timeout ();
		paper::transaction transaction (this_l->connection->node->store.environment, nullptr, true);
		if (!ec)
		{
...
void paper::bulk_push_client::push (MDB_txn * transaction_a)
{
	auto finished (false);
	{
		...
		{
			paper::block_hash hash (first->first.uint256 ());
			if (!hash.is_zero ())
			{
...
paper::pull_info::pull_info (paper::account const & account_a, paper::block_hash const & head_a, paper::block_hash const & end_a) :
account (account_a),
head (head_a),
end (end_a),
attempts (0)
...
paper::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<paper::node> node_a) :
connections (0),
pulling (0),
node (node_a),
account_count (0),
...
paper::bootstrap_attempt::~bootstrap_attempt ()
{
	BOOST_LOG (node->log) << "Exiting bootstrap attempt";
	node->bootstrap_initiator.notify_listeners (false);
}

bool paper::bootstrap_attempt::request_frontier (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		{
			auto client (std::make_shared<paper::frontier_req_client> (connection_l));
			client->run ();
			frontiers = client;
...
void paper::bootstrap_attempt::request_pull (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		pulls.pop_front ();
		auto client (std::make_shared<paper::bulk_pull_client> (connection_l));
...
bool paper::bootstrap_attempt::request_push (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		std::future<bool> future;
		{
			auto client (std::make_shared<paper::bulk_push_client> (connection_l));
			client->start ();
			push = client;
			future = client->promise.get_future ();
		}
...
bool paper::bootstrap_attempt::still_pulling ()
{
...
void paper::bootstrap_attempt::run ()
{
	...
	{
		auto k = paper::random_pool.GenerateWord32 (0, i);
...
std::shared_ptr<paper::bootstrap_client> paper::bootstrap_attempt::connection (std::unique_lock<std::mutex> & lock_a)
{
	while (!stopped && idle.empty ())
	{
		condition.wait (lock_a);
	}
	std::shared_ptr<paper::bootstrap_client> result;
	if (!idle.empty ())
	{
		result = idle.back ();
		idle.pop_back ();
...
bool paper::bootstrap_attempt::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
void paper::bootstrap_attempt::process_fork (MDB_txn * transaction_a, std::shared_ptr<paper::block> block_a)
{
	try_resolve_fork (transaction_a, block_a, true);
}

void paper::bootstrap_attempt::try_resolve_fork (MDB_txn * transaction_a, std::shared_ptr<paper::block> block_a, bool from_processor)
{
	std::weak_ptr<paper::bootstrap_attempt> this_w (shared_from_this ());
	std::shared_ptr<paper::block> ledger_block (node->ledger.forked_block (transaction_a, *block_a));
	if (ledger_block)
	{
		node->active.start (transaction_a, ledger_block, [this_w, block_a](std::shared_ptr<paper::block>, bool resolved) {
			if (auto this_l = this_w.lock ())
			{
				if (resolved)
				{
					...
					paper::transaction transaction (this_l->node->store.environment, nullptr, false);
					auto account (this_l->node->ledger.store.frontier_get (transaction, block_a->root ()));
					if (!account.is_zero ())
					{
						this_l->requeue_pull (paper::pull_info (account, block_a->root (), block_a->root ()));
					}
					else if (this_l->node->ledger.store.account_exists (transaction, block_a->root ()))
					{
						this_l->requeue_pull (paper::pull_info (block_a->root (), paper::block_hash (0), paper::block_hash (0)));
					}
				}
			}
		});
...
void paper::bootstrap_attempt::resolve_forks ()
{
	std::unordered_map<paper::block_hash, std::shared_ptr<paper::block>> forks_to_resolve;
	{
		std::unique_lock<std::mutex> lock (mutex);
...
{
	bool operator() (const std::shared_ptr<paper::bootstrap_client> & lhs, const std::shared_ptr<paper::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

unsigned paper::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
...
void paper::bootstrap_attempt::populate_connections ()
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	std::priority_queue<std::shared_ptr<paper::bootstrap_client>, std::vector<std::shared_ptr<paper::bootstrap_client>>, block_rate_cmp> sorted_connections;
	{
		std::unique_lock<std::mutex> lock (mutex);
	...
	if (connections < target)
	{
		...
		for (int i = 0; i < delta; i++)
		{
			...
			if (peer != paper::endpoint (boost::asio::ip::address_v6::any (), 0))
			{
				auto client (std::make_shared<paper::bootstrap_client> (node, shared_from_this (), paper::tcp_endpoint (peer.address (), peer.port ())));
				client->run ();
...
void paper::bootstrap_attempt::add_connection (paper::endpoint const & endpoint_a)
{
	auto client (std::make_shared<paper::bootstrap_client> (node, shared_from_this (), paper::tcp_endpoint (endpoint_a.address (), endpoint_a.port ())));
	client->run ();
}

void paper::bootstrap_attempt::pool_connection (std::shared_ptr<paper::bootstrap_client> client_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void paper::bootstrap_attempt::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void paper::bootstrap_attempt::add_pull (paper::pull_info const & pull)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void paper::bootstrap_attempt::requeue_pull (paper::pull_info const & pull_a)
{
	auto pull (pull_a);
	...
	else if (pull.attempts == bootstrap_frontier_retry_limit)
	{
		...
		{
			auto client (std::make_shared<paper::bulk_pull_client> (connection_shared));
			node->background ([client, pull]() {
				client->request (pull);
			});
			if (node->config.logging.bulk_pull_logging ())
...
void paper::bootstrap_initiator::bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped && attempt == nullptr)
	{
		attempt = std::make_shared<paper::bootstrap_attempt> (node.shared ());
		condition.notify_all ();
	}
}
...
void paper::bootstrap_initiator::run_bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
...
void paper::bootstrap_initiator::add_observer (std::function<void(bool)> const & observer_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	observers.push_back (observer_a);
}

bool paper::bootstrap_initiator::in_progress ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return attempt != nullptr;
}

void paper::bootstrap_initiator::stop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	stopped = true;
	if (attempt != nullptr)
...
void paper::bootstrap_initiator::notify_listeners (bool in_progress_a)
{
...
void paper::bootstrap_initiator::process_fork (MDB_txn * transaction, std::shared_ptr<paper::block> block_a)
{
	std::unique_lock<std::mutex> lock (mutex);
	if (attempt != nullptr)
	{
	...
	if (!ec)
	{
		...
		if (!paper::message::read_header (type_stream, version_max, version_using, version_min, type, extensions))
		{
			switch (type)
			{
				...
				case paper::message_type::bulk_pull_blocks:
				{
					auto this_l (shared_from_this ());
					boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + paper::bootstrap_message_header_size, sizeof (paper::uint256_union) + sizeof (paper::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t)), [this_l](boost::system::error_code const & ec, size_t size_a) {
...
void paper::bootstrap_server::receive_bulk_pull_blocks_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		std::unique_ptr<paper::bulk_pull_blocks> request (new paper::bulk_pull_blocks);
		paper::bufferstream stream (receive_buffer.data (), 8 + sizeof (paper::uint256_union) + sizeof (paper::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t));
		auto error (request->deserialize (stream));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
...
public:
	...
	{
		auto response (std::make_shared<paper::bulk_pull_blocks_server> (connection, std::unique_ptr<paper::bulk_pull_blocks> (static_cast<paper::bulk_pull_blocks *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
 ...
 */
void paper::bulk_pull_blocks_server::set_params ()
{
	assert (request != nullptr);
	...
	if (connection->node->config.logging.bulk_pull_logging ())
	{
		...
		switch (request->mode)
		{
			case paper::bulk_pull_blocks_mode::list_blocks:
				modeName = "list";
				break;
			case paper::bulk_pull_blocks_mode::checksum_blocks:
				modeName = "checksum";
				break;
		}
...
void paper::bulk_pull_blocks_server::send_next ()
{
	std::unique_ptr<paper::block> block (get_next ());
	if (block != nullptr)
	{
		...
		if (request->mode == paper::bulk_pull_blocks_mode::list_blocks)
		{
			paper::vectorstream stream (send_buffer);
			paper::serialize_block (stream, *block);
		}
		else if (request->mode == paper::bulk_pull_blocks_mode::checksum_blocks)
		{
			checksum ^= block->hash ();
		}
	...
	else
	{
		...
		if (request->mode == paper::bulk_pull_blocks_mode::checksum_blocks)
		{
			{
				send_buffer.clear ();
				paper::vectorstream stream (send_buffer);
				write (stream, static_cast<uint8_t> (paper::block_type::not_a_block));
...
std::unique_ptr<paper::block> paper::bulk_pull_blocks_server::get_next ()
{
	std::unique_ptr<paper::block> result;
	bool out_of_bounds;
...
void paper::bulk_pull_blocks_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void paper::bulk_pull_blocks_server::send_finished ()
{
	send_buffer.clear ();
	send_buffer.push_back (static_cast<uint8_t> (paper::block_type::not_a_block));
	auto this_l (shared_from_this ());
...
void paper::bulk_pull_blocks_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
paper::bulk_pull_blocks_server::bulk_pull_blocks_server (std::shared_ptr<paper::bootstrap_server> const & connection_a, std::unique_ptr<paper::bulk_pull_blocks> request_a) :
connection (connection_a),
request (std::move (request_a)),
stream (nullptr),
...
void paper::frontier_req_server::next ()
{
	...
	if (iterator != connection->node->store.latest_end ())
	{
		current = paper::uint256_union (iterator->first.uint256 ());
		info = paper::account_info (iterator->second);
	}
	else
	{
