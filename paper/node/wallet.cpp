...
void paper::wallet_store::wallet_key (paper::raw_key & prv_a, MDB_txn * transaction_a)
{
	std::lock_guard<std::recursive_mutex> lock (mutex);
	paper::raw_key wallet_l;
...
void paper::wallet_store::seed (paper::raw_key & prv_a, MDB_txn * transaction_a)
{
	paper::wallet_value value (entry_get_raw (transaction_a, paper::wallet_store::seed_special));
	paper::raw_key password_l;
	wallet_key (password_l, transaction_a);
	prv_a.decrypt (value.key, password_l, salt (transaction_a).owords[0]);
}

void paper::wallet_store::seed_set (MDB_txn * transaction_a, paper::raw_key const & prv_a)
{
	paper::raw_key password_l;
	wallet_key (password_l, transaction_a);
	paper::uint256_union ciphertext;
	ciphertext.encrypt (prv_a, password_l, salt (transaction_a).owords[0]);
	entry_put_raw (transaction_a, paper::wallet_store::seed_special, paper::wallet_value (ciphertext, 0));
	deterministic_clear (transaction_a);
}

paper::public_key paper::wallet_store::deterministic_insert (MDB_txn * transaction_a)
{
	auto index (deterministic_index_get (transaction_a));
	paper::raw_key prv;
	deterministic_key (prv, transaction_a, index);
	paper::public_key result;
	...
	marker <<= 32;
	marker |= index;
	entry_put_raw (transaction_a, result, paper::wallet_value (paper::uint256_union (marker), 0));
	++index;
	deterministic_index_set (transaction_a, index);
	return result;
}

void paper::wallet_store::deterministic_key (paper::raw_key & prv_a, MDB_txn * transaction_a, uint32_t index_a)
{
	assert (valid_password (transaction_a));
	paper::raw_key seed_l;
	seed (seed_l, transaction_a);
	paper::deterministic_key (seed_l.data, index_a, prv_a.data);
}

uint32_t paper::wallet_store::deterministic_index_get (MDB_txn * transaction_a)
{
	paper::wallet_value value (entry_get_raw (transaction_a, paper::wallet_store::deterministic_index_special));
	return value.key.number ().convert_to<uint32_t> ();
}

void paper::wallet_store::deterministic_index_set (MDB_txn * transaction_a, uint32_t index_a)
{
	paper::uint256_union index_l (index_a);
	paper::wallet_value value (index_l, 0);
	entry_put_raw (transaction_a, paper::wallet_store::deterministic_index_special, value);
}

void paper::wallet_store::deterministic_clear (MDB_txn * transaction_a)
{
	paper::uint256_union key (0);
	for (auto i (begin (transaction_a)), n (end ()); i != n;)
	{
		switch (key_type (paper::wallet_value (i->second)))
		{
			case paper::key_type::deterministic:
			{
				paper::uint256_union key (i->first.uint256 ());
...
bool paper::wallet_store::valid_password (MDB_txn * transaction_a)
{
	paper::raw_key zero;
	zero.data.clear ();
	paper::raw_key wallet_key_l;
	wallet_key (wallet_key_l, transaction_a);
	paper::uint256_union check_l;
	check_l.encrypt (zero, wallet_key_l, salt (transaction_a).owords[0]);
	bool ok = check (transaction_a) == check_l;
	return ok;
}

bool paper::wallet_store::attempt_password (MDB_txn * transaction_a, std::string const & password_a)
{
	bool result = false;
	{
		std::lock_guard<std::recursive_mutex> lock (mutex);
...
bool paper::wallet_store::rekey (MDB_txn * transaction_a, std::string const & password_a)
{
	...
	if (valid_password (transaction_a))
	{
		...
		paper::raw_key wallet_enc;
		wallet_enc.data = encrypted;
		wallet_key_mem.value_set (wallet_enc);
		entry_put_raw (transaction_a, paper::wallet_store::wallet_key_special, paper::wallet_value (encrypted, 0));
	}
	else
	{
		result = true;
...
void paper::wallet_store::derive_key (paper::raw_key & prv_a, MDB_txn * transaction_a, std::string const & password_a)
{
...
void paper::fan::value (paper::raw_key & prv_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	value_get (prv_a);
}

void paper::fan::value_get (paper::raw_key & prv_a)
{
...
void paper::fan::value_set (paper::raw_key const & value_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	paper::raw_key value_l;
	value_get (value_l);
	*(values[0]) ^= value_l.data;
	*(values[0]) ^= value_a.data;
}

paper::wallet_value::wallet_value (paper::mdb_val const & val_a)
{
...
paper::wallet_value::wallet_value (paper::uint256_union const & key_a, uint64_t work_a) :
key (key_a),
work (work_a)
{
}
...
unsigned const paper::wallet_store::version_1 (1);
unsigned const paper::wallet_store::version_2 (2);
unsigned const paper::wallet_store::version_3 (3);
unsigned const paper::wallet_store::version_current (version_3);
...
paper::uint256_union const paper::wallet_store::seed_special (5);
// Current key index for deterministic keys
paper::uint256_union const paper::wallet_store::deterministic_index_special (6);
int const paper::wallet_store::special_count (7);

paper::wallet_store::wallet_store (bool & init_a, paper::kdf & kdf_a, paper::transaction & transaction_a, paper::account representative_a, unsigned fanout_a, std::string const & wallet_a, std::string const & json_a) :
password (0, fanout_a),
...
environment (transaction_a.environment)
{
	...
	{
		MDB_val junk;
		assert (mdb_get (transaction_a, handle, paper::mdb_val (version_special), &junk) == MDB_NOTFOUND);
		...
		for (auto i (wallet_l.begin ()), n (wallet_l.end ()); i != n; ++i)
		{
			...
			if (!init_a)
			{
				...
				if (!init_a)
				{
					entry_put_raw (transaction_a, key, paper::wallet_value (value, 0));
				}
				else
				{
					init_a = true;
		...
		init_a |= mdb_get (transaction_a, handle, paper::mdb_val (version_special), &junk) != 0;
		init_a |= mdb_get (transaction_a, handle, paper::mdb_val (wallet_key_special), &junk) != 0;
		init_a |= mdb_get (transaction_a, handle, paper::mdb_val (salt_special), &junk) != 0;
		init_a |= mdb_get (transaction_a, handle, paper::mdb_val (check_special), &junk) != 0;
		init_a |= mdb_get (transaction_a, handle, paper::mdb_val (representative_special), &junk) != 0;
		paper::raw_key key;
		key.data.clear ();
		password.value_set (key);
		key.data = entry_get_raw (transaction_a, paper::wallet_store::wallet_key_special).key;
		wallet_key_mem.value_set (key);
	}
}

paper::wallet_store::wallet_store (bool & init_a, paper::kdf & kdf_a, paper::transaction & transaction_a, paper::account representative_a, unsigned fanout_a, std::string const & wallet_a) :
password (0, fanout_a),
wallet_key_mem (0, fanout_a),
kdf (kdf_a),
environment (transaction_a.environment)
{
	...
	if (!init_a)
	{
		...
		version_status = mdb_get (transaction_a, handle, paper::mdb_val (version_special), &version_value);
		if (version_status == MDB_NOTFOUND)
		{
			...
			entry_put_raw (transaction_a, paper::wallet_store::salt_special, paper::wallet_value (salt_l, 0));
			// Wallet key is a fixed random key that encrypts all entries
			paper::raw_key wallet_key;
			...
			entry_put_raw (transaction_a, paper::wallet_store::check_special, paper::wallet_value (check, 0));
			entry_put_raw (transaction_a, paper::wallet_store::representative_special, paper::wallet_value (representative_a, 0));
			paper::raw_key seed;
			random_pool.GenerateBlock (seed.data.bytes.data (), seed.data.bytes.size ());
			seed_set (transaction_a, seed);
			entry_put_raw (transaction_a, paper::wallet_store::deterministic_index_special, paper::wallet_value (paper::uint256_union (0), 0));
		}
	}
	paper::raw_key key;
	key.data = entry_get_raw (transaction_a, paper::wallet_store::wallet_key_special).key;
...
paper::public_key paper::wallet_store::insert_adhoc (MDB_txn * transaction_a, paper::raw_key const & prv)
{
	assert (valid_password (transaction_a));
	...
	entry_put_raw (transaction_a, pub, paper::wallet_value (ciphertext, 0));
	return pub;
}

void paper::wallet_store::erase (MDB_txn * transaction_a, paper::public_key const & pub)
{
	auto status (mdb_del (transaction_a, handle, paper::mdb_val (pub), nullptr));
	assert (status == 0);
}
...
paper::wallet_value paper::wallet_store::entry_get_raw (MDB_txn * transaction_a, paper::public_key const & pub_a)
{
	...
	paper::mdb_val value;
	auto status (mdb_get (transaction_a, handle, paper::mdb_val (pub_a), value));
	if (status == 0)
	{
...
{
	auto status (mdb_put (transaction_a, handle, paper::mdb_val (pub_a), entry_a.val (), 0));
	assert (status == 0);
}

paper::key_type paper::wallet_store::key_type (paper::wallet_value const & value_a)
{
	auto number (value_a.key.number ());
	paper::key_type result;
	auto text (number.convert_to<std::string> ());
	if (number > std::numeric_limits<uint64_t>::max ())
	{
		result = paper::key_type::adhoc;
	}
	else
	{
		if ((number >> 32).convert_to<uint32_t> () == 1)
		{
			result = paper::key_type::deterministic;
		}
		else
		{
			result = paper::key_type::unknown;
		}
	}
	return result;
}

bool paper::wallet_store::fetch (MDB_txn * transaction_a, paper::public_key const & pub, paper::raw_key & prv)
{
	auto result (false);
	if (valid_password (transaction_a))
	{
...
void paper::wallet_store::serialize_json (MDB_txn * transaction_a, std::string & string_a)
{
	...
	{
		tree.put (paper::uint256_union (i->first.uint256 ()).to_string (), paper::wallet_value (i->second).key.to_string ());
	}
...
unsigned paper::wallet_store::version (MDB_txn * transaction_a)
{
	paper::wallet_value value (entry_get_raw (transaction_a, paper::wallet_store::version_special));
	auto entry (value.key);
	auto result (static_cast<unsigned> (entry.bytes[31]));
	return result;
}

void paper::wallet_store::version_put (MDB_txn * transaction_a, unsigned version_a)
{
	paper::uint256_union entry (version_a);
	entry_put_raw (transaction_a, paper::wallet_store::version_special, paper::wallet_value (entry, 0));
}

void paper::wallet_store::upgrade_v1_v2 ()
{
	paper::transaction transaction (environment, nullptr, true);
	assert (version (transaction) == 1);
	paper::raw_key zero_password;
	paper::wallet_value value (entry_get_raw (transaction, paper::wallet_store::wallet_key_special));
	paper::raw_key kdf;
	kdf.data.clear ();
	zero_password.decrypt (value.key, kdf, salt (transaction).owords[0]);
	derive_key (kdf, transaction, "");
	paper::raw_key empty_password;
	empty_password.decrypt (value.key, kdf, salt (transaction).owords[0]);
	for (auto i (begin (transaction)), n (end ()); i != n; ++i)
	{
		paper::public_key key (i->first.uint256 ());
		paper::raw_key prv;
		if (fetch (transaction, key, prv))
		{
			// Key failed to decrypt despite valid password
			paper::wallet_value data (entry_get_raw (transaction, key));
...
void paper::wallet_store::upgrade_v2_v3 ()
{
	paper::transaction transaction (environment, nullptr, true);
	...
	entry_put_raw (transaction, paper::wallet_store::deterministic_index_special, paper::wallet_value (paper::uint256_union (0), 0));
	version_put (transaction, 3);
}

void paper::kdf::phs (paper::raw_key & result_a, std::string const & password_a, paper::uint256_union const & salt_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto success (argon2_hash (1, paper::wallet_store::kdf_work, 1, password_a.data (), password_a.size (), salt_a.bytes.data (), salt_a.bytes.size (), result_a.data.bytes.data (), result_a.data.bytes.size (), NULL, 0, Argon2_d, 0x10));
	assert (success == 0);
	(void)success;
}
...
void paper::wallet::enter_initial_password ()
{
...
bool paper::wallet::enter_password (std::string const & password_a)
{
	paper::transaction transaction (store.environment, nullptr, false);
...
paper::public_key paper::wallet::deterministic_insert (MDB_txn * transaction_a, bool generate_work_a)
{
	paper::public_key key (0);
...
paper::public_key paper::wallet::deterministic_insert (bool generate_work_a)
{
	paper::transaction transaction (store.environment, nullptr, true);
...
paper::public_key paper::wallet::insert_adhoc (MDB_txn * transaction_a, paper::raw_key const & key_a, bool generate_work_a)
{
	paper::public_key key (0);
	if (store.valid_password (transaction_a))
	{
...
paper::public_key paper::wallet::insert_adhoc (paper::raw_key const & account_a, bool generate_work_a)
{
	paper::transaction transaction (store.environment, nullptr, true);
...
{
	auto error (false);
	std::unique_ptr<paper::wallet_store> temp;
	{
		paper::transaction transaction (store.environment, nullptr, true);
		paper::uint256_union id;
		random_pool.GenerateBlock (id.bytes.data (), id.bytes.size ());
		temp.reset (new paper::wallet_store (error, node.wallets.kdf, transaction, 0, 1, id.to_string (), json_a));
	}
	if (!error)
	{
		paper::transaction transaction (store.environment, nullptr, false);
...
std::shared_ptr<paper::block> paper::wallet::receive_action (paper::send_block const & send_a, paper::account const & representative_a, paper::uint128_union const & amount_a, bool generate_work_a)
{
	...
	if (node.config.receive_minimum.number () <= amount_a.number ())
	{
		...
		if (node.ledger.store.pending_exists (transaction, paper::pending_key (send_a.hashables.destination, hash)))
		{
			paper::raw_key prv;
			if (!store.fetch (transaction, send_a.hashables.destination, prv))
			{
				...
				if (!new_account)
				{
					auto receive (new paper::receive_block (info.head, hash, prv, send_a.hashables.destination, generate_work_a ? work_fetch (transaction, send_a.hashables.destination, info.head) : 0));
					block.reset (receive);
				}
				else
				{
					block.reset (new paper::open_block (hash, representative_a, send_a.hashables.destination, prv, send_a.hashables.destination, generate_work_a ? work_fetch (transaction, send_a.hashables.destination, send_a.hashables.destination) : 0));
				}
			}
			else
			{
	...
	if (block != nullptr)
	{
		...
		if (generate_work_a)
		{
			...
			node.wallets.queue_wallet_action (paper::wallets::generate_priority, [this_l, source, hash] {
...
std::shared_ptr<paper::block> paper::wallet::change_action (paper::account const & source_a, paper::account const & representative_a, bool generate_work_a)
{
	std::shared_ptr<paper::block> block;
	{
		paper::transaction transaction (store.environment, nullptr, false);
		if (store.valid_password (transaction))
		{
			...
			if (existing != store.end () && !node.ledger.latest (transaction, source_a).is_zero ())
			{
				...
				assert (!error2);
				block.reset (new paper::change_block (info.head, representative_a, prv, source_a, generate_work_a ? work_fetch (transaction, source_a, info.head) : 0));
			}
		}
	}
	if (block != nullptr)
	{
		...
		if (generate_work_a)
		{
			auto hash (block->hash ());
			auto this_l (shared_from_this ());
			node.wallets.queue_wallet_action (paper::wallets::generate_priority, [this_l, source_a, hash] {
...
std::shared_ptr<paper::block> paper::wallet::send_action (paper::account const & source_a, paper::account const & account_a, paper::uint128_t const & amount_a, bool generate_work_a, boost::optional<std::string> id_a)
{
	std::shared_ptr<paper::block> block;
	boost::optional<paper::mdb_val> id_mdb_val;
	if (id_a)
	{
		id_mdb_val = paper::mdb_val (id_a->size (), const_cast<char *> (id_a->data ()));
	}
	bool error = false;
	bool cached_block = false;
	{
		paper::transaction transaction (store.environment, nullptr, (bool)id_mdb_val);
		if (id_mdb_val)
		{
...
bool paper::wallet::change_sync (paper::account const & source_a, paper::account const & representative_a)
{
	...
	change_async (source_a, representative_a, [this, source_a, representative_a, &result](std::shared_ptr<paper::block> block_a) {
...
void paper::wallet::change_async (paper::account const & source_a, paper::account const & representative_a, std::function<void(std::shared_ptr<paper::block>)> const & action_a, bool generate_work_a)
{
	node.wallets.queue_wallet_action (paper::wallets::high_priority, [this, source_a, representative_a, action_a, generate_work_a]() {
		auto block (change_action (source_a, representative_a, generate_work_a));
		action_a (block);
	});
}

bool paper::wallet::receive_sync (std::shared_ptr<paper::block> block_a, paper::account const & representative_a, paper::uint128_t const & amount_a)
{
	std::promise<bool> result;
	receive_async (block_a, representative_a, amount_a, [&result](std::shared_ptr<paper::block> block_a) {
		result.set_value (block_a == nullptr);
	},
	true);
	return result.get_future ().get ();
...
void paper::wallet::receive_async (std::shared_ptr<paper::block> block_a, paper::account const & representative_a, paper::uint128_t const & amount_a, std::function<void(std::shared_ptr<paper::block>)> const & action_a, bool generate_work_a)
{
	assert (dynamic_cast<paper::send_block *> (block_a.get ()) != nullptr);
	node.wallets.queue_wallet_action (amount_a, [this, block_a, representative_a, amount_a, action_a, generate_work_a]() {
		auto block (receive_action (*static_cast<paper::send_block *> (block_a.get ()), representative_a, amount_a, generate_work_a));
		action_a (block);
	});
}

paper::block_hash paper::wallet::send_sync (paper::account const & source_a, paper::account const & account_a, paper::uint128_t const & amount_a)
{
	std::promise<paper::block_hash> result;
	send_async (source_a, account_a, amount_a, [&result](std::shared_ptr<paper::block> block_a) {
...
void paper::wallet::send_async (paper::account const & source_a, paper::account const & account_a, paper::uint128_t const & amount_a, std::function<void(std::shared_ptr<paper::block>)> const & action_a, bool generate_work_a, boost::optional<std::string> id_a)
{
	node.background ([this, source_a, account_a, amount_a, action_a, generate_work_a, id_a]() {
		this->node.wallets.queue_wallet_action (paper::wallets::high_priority, [this, source_a, account_a, amount_a, action_a, generate_work_a, id_a]() {
...
uint64_t paper::wallet::work_fetch (MDB_txn * transaction_a, paper::account const & account_a, paper::block_hash const & root_a)
{
	...
	else if (paper::work_validate (root_a, result))
	{
...
void paper::wallet::work_ensure (MDB_txn * transaction_a, paper::account const & account_a)
{
	assert (store.exists (transaction_a, account_a));
...
public:
	...
	void run ()
	{
		...
		std::unordered_set<paper::account> already_searched;
		for (auto i (wallet->node.store.pending_begin (transaction)), n (wallet->node.store.pending_end ()); i != n; ++i)
		{
			paper::pending_key key (i->first);
			paper::pending_info pending (i->second);
			...
			if (existing != keys.end ())
			{
				...
				if (wallet->node.config.receive_minimum.number () <= amount)
				{
					...
					{
						auto this_l (shared_from_this ());
						std::shared_ptr<paper::block> block_l (wallet->node.store.block_get (transaction, info.head));
						wallet->node.background ([this_l, account, block_l] {
							paper::transaction transaction (this_l->wallet->node.store.environment, nullptr, true);
							this_l->wallet->node.active.start (transaction, block_l, [this_l, account](std::shared_ptr<paper::block>, bool) {
		...
		for (auto i (wallet->node.store.pending_begin (transaction)), n (wallet->node.store.pending_end ()); i != n; ++i)
		{
			...
			if (pending.source == account_a)
			{
				if (wallet->store.exists (transaction, key.account))
				{
					...
					{
						paper::pending_key key (i->first);
						std::shared_ptr<paper::block> block (wallet->node.store.block_get (transaction, key.hash));
						auto wallet_l (wallet);
						auto amount (pending.amount.number ());
						BOOST_LOG (wallet_l->node.log) << boost::str (boost::format ("Receiving block: %1%") % block->hash ().to_string ());
						wallet_l->receive_async (block, representative, amount, [wallet_l, block](std::shared_ptr<paper::block> block_a) {
							if (block_a == nullptr)
							{
...
void paper::wallet::init_free_accounts (MDB_txn * transaction_a)
{
	free_accounts.clear ();
...
paper::public_key paper::wallet::change_seed (MDB_txn * transaction_a, paper::raw_key const & prv_a)
{
	...
	for (uint32_t i (1), n (32); i < n; ++i)
	{
		...
		paper::keypair pair (prv.data.to_string ());
...
std::shared_ptr<paper::wallet> paper::wallets::create (paper::uint256_union const & id_a)
{
	...
	{
		paper::transaction transaction (node.store.environment, nullptr, true);
		result = std::make_shared<paper::wallet> (error, transaction, node, id_a.to_string ());
	}
	if (!error)
	{
		items[id_a] = result;
...
void paper::wallets::search_pending_all ()
{
	for (auto i : items)
	{
...
void paper::wallets::do_wallet_actions ()
{
...
void paper::wallets::queue_wallet_action (paper::uint128_t const & amount_a, std::function<void()> const & action_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void paper::wallets::foreach_representative (MDB_txn * transaction_a, std::function<void(paper::public_key const & pub_a, paper::raw_key const & prv_a)> const & action_a)
{
	for (auto i (items.begin ()), n (items.end ()); i != n; ++i)
	{
		...
		{
			paper::account account (j->first.uint256 ());
...
bool paper::wallets::exists (MDB_txn * transaction_a, paper::public_key const & account_a)
{
	auto result (false);
...
paper::uint128_t const paper::wallets::generate_priority = std::numeric_limits<paper::uint128_t>::max ();
paper::uint128_t const paper::wallets::high_priority = std::numeric_limits<paper::uint128_t>::max () - 1;

paper::store_iterator paper::wallet_store::begin (MDB_txn * transaction_a)
{
	paper::store_iterator result (transaction_a, handle, paper::mdb_val (paper::uint256_union (special_count)));
	return result;
}

paper::store_iterator paper::wallet_store::begin (MDB_txn * transaction_a, paper::uint256_union const & key)
{
	paper::store_iterator result (transaction_a, handle, paper::mdb_val (key));
	return result;
}
...
paper::store_iterator paper::wallet_store::find (MDB_txn * transaction_a, paper::uint256_union const & key)
{
	...
	if (result != end)
	{
		if (paper::uint256_union (result->first.uint256 ()) == key)
		{
			return result;
		}
		else
