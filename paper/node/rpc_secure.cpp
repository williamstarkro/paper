...
bool paper::rpc_secure::on_verify_certificate (bool preverified, boost::asio::ssl::verify_context & ctx)
{
...
void paper::rpc_secure::load_certs (boost::asio::ssl::context & context_a)
{
	...
	if (config.secure.client_certs_path.size () > 0)
	{
		...
		context_a.set_verify_callback (boost::bind (&paper::rpc_secure::on_verify_certificate, this, _1, _2));
	}
}

paper::rpc_secure::rpc_secure (boost::asio::io_service & service_a, paper::node & node_a, paper::rpc_config const & config_a) :
...
void paper::rpc_secure::accept ()
{
	auto connection (std::make_shared<paper::rpc_connection_secure> (node, *this));
...
paper::rpc_connection_secure::rpc_connection_secure (paper::node & node_a, paper::rpc_secure & rpc_a) :
paper::rpc_connection (node_a, rpc_a),
stream (socket, rpc_a.ssl_context)
{
}

void paper::rpc_connection_secure::parse_connection ()
{
	// Perform the SSL handshake
	stream.async_handshake (boost::asio::ssl::stream_base::server,
	std::bind (
	&paper::rpc_connection_secure::handle_handshake,
	std::static_pointer_cast<paper::rpc_connection_secure> (shared_from_this ()),
	std::placeholders::_1));
}

void paper::rpc_connection_secure::on_shutdown (const boost::system::error_code & error)
{
...
void paper::rpc_connection_secure::handle_handshake (const boost::system::error_code & error)
{
	if (!error)
	{
		read ();
...
void paper::rpc_connection_secure::read ()
{
	auto this_l (std::static_pointer_cast<paper::rpc_connection_secure> (shared_from_this ()));
			...
			this_l->node->background ([this_l]() {
				...
				auto response_handler ([this_l, version, start](boost::property_tree::ptree const & tree_a) {
					...
					boost::beast::http::async_write (this_l->stream, this_l->res, [this_l](boost::system::error_code const & ec, size_t bytes_transferred) {
						...
						std::bind (
						&paper::rpc_connection_secure::on_shutdown,
						this_l,
						std::placeholders::_1));
