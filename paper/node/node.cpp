...
int constexpr paper::port_mapping::mapping_timeout;
int constexpr paper::port_mapping::check_timeout;
unsigned constexpr paper::active_transactions::announce_interval_ms;

paper::message_statistics::message_statistics () :
keepalive (0),
publish (0),
confirm_req (0),
confirm_ack (0)
...
paper::network::network (paper::node & node_a, uint16_t port) :
socket (node_a.service, paper::endpoint (boost::asio::ip::address_v6::any (), port)),
...
void paper::network::send_keepalive (paper::endpoint const & endpoint_a)
{
	...
	++outgoing.keepalive;
	std::weak_ptr<paper::node> node_w (node.shared ());
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a](boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			for (auto i (i_a), n (boost::asio::ip::udp::resolver::iterator{}); i != n; ++i)
			{
				...
				{
					endpoint = paper::endpoint (boost::asio::ip::address_v6::v4_mapped (endpoint.address ().to_v4 ()), endpoint.port ());
				}
...
void paper::network::republish (paper::block_hash const & hash_a, std::shared_ptr<std::vector<uint8_t>> buffer_a, paper::endpoint endpoint_a)
{
	++outgoing.publish;
...
void paper::network::rebroadcast_reps (std::shared_ptr<paper::block> block_a)
{
	auto hash (block_a->hash ());
	paper::publish message (block_a);
	std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
	{
		paper::vectorstream stream (*bytes);
		message.serialize (stream);
...
template <typename T>
bool confirm_block (MDB_txn * transaction_a, paper::node & node_a, T & list_a, std::shared_ptr<paper::block> block_a)
{
	bool result (false);
	if (node_a.config.enable_voting)
	{
		node_a.wallets.foreach_representative (transaction_a, [&result, &block_a, &list_a, &node_a, &transaction_a](paper::public_key const & pub_a, paper::raw_key const & prv_a) {
			result = true;
...
template <>
bool confirm_block (MDB_txn * transaction_a, paper::node & node_a, paper::endpoint & peer_a, std::shared_ptr<paper::block> block_a)
{
	std::array<paper::endpoint, 1> endpoints;
	endpoints[0] = peer_a;
	auto result (confirm_block (transaction_a, node_a, endpoints, std::move (block_a)));
	return result;
}

void paper::network::republish_block (MDB_txn * transaction, std::shared_ptr<paper::block> block)
{
...
void paper::network::republish_vote (std::chrono::steady_clock::time_point const & last_vote, std::shared_ptr<paper::vote> vote_a)
{
	if (last_vote < std::chrono::steady_clock::now () - std::chrono::seconds (1))
	{
		if (node.weight (vote_a->account) > paper::Mxrb_ratio * 256)
		{
			paper::confirm_ack confirm (vote_a);
			std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
			{
				paper::vectorstream stream (*bytes);
				confirm.serialize (stream);
...
void paper::network::broadcast_confirm_req (std::shared_ptr<paper::block> block_a)
{
...
void paper::network::send_confirm_req (paper::endpoint const & endpoint_a, std::shared_ptr<paper::block> block)
{
	paper::confirm_req message (block);
	std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
	{
		paper::vectorstream stream (*bytes);
		message.serialize (stream);
...
template <typename T>
void rep_query (paper::node & node_a, T const & peers_a)
{
	paper::transaction transaction (node_a.store.environment, nullptr, false);
	std::shared_ptr<paper::block> block (node_a.store.block_random (transaction));
	auto hash (block->hash ());
	...
	std::weak_ptr<paper::node> node_w (node_a.shared ());
...
template <>
void rep_query (paper::node & node_a, paper::endpoint const & peers_a)
{
	std::array<paper::endpoint, 1> peers;
	peers[0] = peers_a;
...
public:
	...
	void confirm_req (paper::confirm_req const & message_a) override
	{
		...
		node.peers.insert (sender, message_a.version_using);
		node.process_active (message_a.block);
		paper::transaction transaction_a (node.store.environment, nullptr, false);
	...
	void confirm_ack (paper::confirm_ack const & message_a) override
	{
		...
		if (vote.code == paper::vote_code::replay)
		{
			...
			{
				paper::confirm_ack confirm (vote.vote);
				std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
				{
					paper::vectorstream stream (*bytes);
					confirm.serialize (stream);
	...
	void bulk_pull_blocks (paper::bulk_pull_blocks const &) override
	{
		assert (false);
	}
...
void paper::network::receive_action (boost::system::error_code const & error, size_t size_a)
{
	if (!error && on)
	{
		if (!paper::reserved_address (remote) && remote != endpoint ())
		{
			...
			paper::message_parser parser (visitor, node.work);
...
paper::alarm::alarm (boost::asio::io_service & service_a) :
service (service_a),
thread ([this]() { run (); })
{
}
...
void paper::alarm::add (std::chrono::steady_clock::time_point const & wakeup_a, std::function<void()> const & operation)
{
...
void paper::logging::init (boost::filesystem::path const & application_path_a)
{
...
bool paper::logging::upgrade_json (unsigned version_a, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	switch (version_a)
	{
...
bool paper::logging::deserialize_json (bool & upgraded_a, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	try
	{
...
bool paper::logging::vote_logging () const
{
...
bool paper::logging::callback_logging () const
{
...
node_config (paper::network::node_port, paper::logging ())
{
}
...
receive_minimum (paper::xrb_ratio),
inactive_supply (0),
...
lmdb_max_dbs (128)
{
	switch (paper::paper_network)
	{
		case paper::paper_networks::paper_test_network:
			preconfigured_representatives.push_back (paper::genesis_account);
			break;
		case paper::paper_networks::paper_beta_network:
			preconfigured_peers.push_back ("paper.paperblocks.net");
			preconfigured_representatives.push_back (paper::account ("59750C057F42806F40C5D9EAA1E0263E9DB48FE385BD0172BFC573BD37EEC4A7"));
			preconfigured_representatives.push_back (paper::account ("8B05C9B160DE9B006FA27DD6A368D7CA122A2EE7537C308CF22EFD3ABF5B36C3"));
			preconfigured_representatives.push_back (paper::account ("91D51BF05F02698EBB4649FB06D1BBFD2E4AE2579660E8D784A002D9C0CB1BD2"));
			preconfigured_representatives.push_back (paper::account ("CB35ED23D47E1A16667EDE415CD4CD05961481D7D23A43958FAE81FC12FA49FF"));
			break;
		case paper::paper_networks::paper_live_network:
			preconfigured_peers.push_back ("paper.paperblocks.net");
			preconfigured_representatives.push_back (paper::account ("A30E0A32ED41C8607AA9212843392E853FCBCB4E7CB194E35C94F07F91DE59EF"));
			preconfigured_representatives.push_back (paper::account ("67556D31DDFC2A440BF6147501449B4CB9572278D034EE686A6BEE29851681DF"));
			preconfigured_representatives.push_back (paper::account ("5C2FBB148E006A8E8BA7A75DD86C9FE00C83F5FFDBFD76EAA09531071436B6AF"));
			preconfigured_representatives.push_back (paper::account ("AE7AC63990DAAAF2A69BF11C913B928844BF5012355456F2F164166464024B29"));
			preconfigured_representatives.push_back (paper::account ("BD6267D6ECD8038327D2BCC0850BDF8F56EC0414912207E81BCF90DFAC8A4AAA"));
			preconfigured_representatives.push_back (paper::account ("2399A083C600AA0572F5E36247D978FCFC840405F8D4B6D33161C0066A55F431"));
			preconfigured_representatives.push_back (paper::account ("2298FAB7C61058E77EA554CB93EDEEDA0692CBFCC540AB213B2836B29029E23A"));
			preconfigured_representatives.push_back (paper::account ("3FE80B4BC842E82C1C18ABFEEC47EA989E63953BC82AC411F304D13833D52A56"));
			break;
		default:
			assert (false);
			break;
...
bool paper::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	switch (version)
	{
		...
		case 2:
		{
			tree_a.put ("inactive_supply", paper::uint128_union (0).to_string_dec ());
		...
		case 3:
			...
			tree_a.put ("receive_minimum", paper::xrb_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
...
bool paper::node_config::deserialize_json (bool & upgraded_a, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	try
	{
		...
		for (auto i (work_peers_l.begin ()), n (work_peers_l.end ()); i != n; ++i)
		{
			...
			uint16_t port;
			result |= paper::parse_address_port (work_peer, address, port);
			if (!result)
			{
		...
		{
			paper::account representative (0);
...
paper::account paper::node_config::random_representative ()
{
	assert (preconfigured_representatives.size () > 0);
	size_t index (paper::random_pool.GenerateWord32 (0, preconfigured_representatives.size () - 1));
	auto result (preconfigured_representatives[index]);
	return result;
}

paper::vote_processor::vote_processor (paper::node & node_a) :
node (node_a)
{
}

paper::vote_result paper::vote_processor::vote (std::shared_ptr<paper::vote> vote_a, paper::endpoint endpoint_a)
{
	paper::vote_result result;
	{
	...
	if (node.config.logging.vote_logging ())
	{
		...
		switch (result.code)
		{
			case paper::vote_code::invalid:
				status = "Invalid";
				break;
			case paper::vote_code::replay:
				status = "Replay";
				break;
			case paper::vote_code::vote:
				status = "Vote";
				break;
		}
...
void paper::rep_crawler::add (paper::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	active.insert (hash_a);
}

void paper::rep_crawler::remove (paper::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	active.erase (hash_a);
}

bool paper::rep_crawler::exists (paper::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	return active.count (hash_a) != 0;
}

paper::block_processor_item::block_processor_item (std::shared_ptr<paper::block> block_a) :
block_processor_item (block_a, false)
{
}

paper::block_processor_item::block_processor_item (std::shared_ptr<paper::block> block_a, bool force_a) :
block (block_a),
force (force_a)
{
}

paper::block_processor::block_processor (paper::node & node_a) :
stopped (false),
idle (true),
node (node_a)
{
...
void paper::block_processor::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void paper::block_processor::flush ()
{
	std::unique_lock<std::mutex> lock (mutex);
...
void paper::block_processor::add (paper::block_processor_item const & item_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void paper::block_processor::process_blocks ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
		if (!blocks.empty ())
		{
			std::deque<paper::block_processor_item> blocks_processing;
...
void paper::block_processor::process_receive_many (paper::block_processor_item const & item_a)
{
	std::deque<paper::block_processor_item> blocks_processing;
...
void paper::block_processor::process_receive_many (std::deque<paper::block_processor_item> & blocks_processing)
{
	while (!blocks_processing.empty ())
	{
		std::deque<std::pair<std::shared_ptr<paper::block>, paper::process_return>> progress;
		{
			paper::transaction transaction (node.store.environment, nullptr, true);
			auto cutoff (std::chrono::steady_clock::now () + paper::transaction_timeout);
				...
				switch (process_result.code)
				{
					...
					case paper::process_result::old:
					{
						...
						for (auto i (cached.begin ()), n (cached.end ()); i != n; ++i)
						{
							...
							blocks_processing.push_front (paper::block_processor_item (*i));
						}
...
paper::process_return paper::block_processor::process_receive_one (MDB_txn * transaction_a, std::shared_ptr<paper::block> block_a)
{
	...
	switch (result.code)
	{
		...
		{
			{
				...
				if (existing != nullptr)
				{
					...
					if (paper::work_value (root, block_a->block_work ()) > paper::work_value (root, existing->block_work ()))
					{
						auto account (node.ledger.account (transaction_a, hash));
						if (!paper::validate_message (account, hash, block_a->block_signature ()))
						{
		...
		case paper::process_result::negative_spend:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case paper::process_result::opened_burn_account:
		{
...
paper::node::node (paper::node_init & init_a, boost::asio::io_service & service_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, paper::alarm & alarm_a, paper::logging const & logging_a, paper::work_pool & work_a) :
node (init_a, service_a, application_path_a, alarm_a, paper::node_config (peering_port_a, logging_a), work_a)
{
}

paper::node::node (paper::node_init & init_a, boost::asio::io_service & service_a, boost::filesystem::path const & application_path_a, paper::alarm & alarm_a, paper::node_config const & config_a, paper::work_pool & work_a) :
service (service_a),
config (config_a),
alarm (alarm_a),
work (work_a),
...
block_processor_thread ([this]() { this->block_processor.process_blocks (); })
{
	...
	observers.endpoint.add ([this](paper::endpoint const & endpoint_a) {
		this->network.send_keepalive (endpoint_a);
		rep_query (*this, endpoint_a);
	});
	observers.vote.add ([this](std::shared_ptr<paper::vote> vote_a, paper::endpoint const &) {
		active.vote (vote_a);
	});
	observers.vote.add ([this](std::shared_ptr<paper::vote> vote_a, paper::endpoint const &) {
		this->gap_cache.vote (vote_a);
	});
	observers.vote.add ([this](std::shared_ptr<paper::vote> vote_a, paper::endpoint const & endpoint_a) {
	...
	BOOST_LOG (log) << "Node starting, version: " << PAPER_VERSION_MAJOR << "." << PAPER_VERSION_MINOR;
	...
	if (paper::paper_network == paper::paper_networks::paper_live_network)
	{
		extern const char paper_bootstrap_weights[];
		extern const size_t paper_bootstrap_weights_size;
		paper::bufferstream weight_stream ((const uint8_t *)paper_bootstrap_weights, paper_bootstrap_weights_size);
		paper::uint128_union block_height;
		if (!paper::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					if (paper::read (weight_stream, account.bytes))
					{
						break;
					}
					paper::amount weight;
...
bool paper::node::copy_with_compaction (boost::filesystem::path const & destination_file)
{
...
void paper::gap_cache::add (MDB_txn * transaction_a, std::shared_ptr<paper::block> block_a)
{
	auto hash (block_a->hash ());
	std::lock_guard<std::mutex> lock (mutex);
	...
	{
		blocks.get<1> ().modify (existing, [](paper::gap_information & info) {
			info.arrival = std::chrono::steady_clock::now ();
		});
	}
	else
	{
		blocks.insert ({ std::chrono::steady_clock::now (), hash, std::unique_ptr<paper::votes> (new paper::votes (block_a)) });
		if (blocks.size () > max)
		{
...
void paper::gap_cache::vote (std::shared_ptr<paper::vote> vote_a)
{
	paper::transaction transaction (node.store.environment, nullptr, false);
	std::lock_guard<std::mutex> lock (mutex);
	...
	if (existing != blocks.get<1> ().end ())
	{
		...
		if (winner.first > bootstrap_threshold (transaction))
		{
			...
			node.alarm.add (paper::paper_network == paper::paper_networks::paper_test_network ? now + std::chrono::milliseconds (5) : now + std::chrono::seconds (5), [node_l, hash]() {
...
void paper::gap_cache::purge_old ()
{
...
void paper::network::confirm_send (paper::confirm_ack const & confirm_a, std::shared_ptr<std::vector<uint8_t>> bytes_a, paper::endpoint const & endpoint_a)
{
	if (node.config.logging.network_publish_logging ())
	{
...
void paper::node::process_active (std::shared_ptr<paper::block> incoming)
{
...
std::vector<paper::endpoint> paper::peer_container::list_sqrt ()
{
	auto peers (random_set (2 * size_sqrt ()));
	std::vector<paper::endpoint> result;
...
std::vector<paper::endpoint> paper::peer_container::list ()
{
	std::vector<paper::endpoint> result;
	std::lock_guard<std::mutex> lock (mutex);
	result.reserve (peers.size ());
...
std::map<paper::endpoint, unsigned> paper::peer_container::list_version ()
{
	std::map<paper::endpoint, unsigned> result;
	std::lock_guard<std::mutex> lock (mutex);
	for (auto i (peers.begin ()), j (peers.end ()); i != j; ++i)
	{
		result.insert (std::pair<paper::endpoint, unsigned> (i->endpoint, i->network_version));
	}
	return result;
}

paper::endpoint paper::peer_container::bootstrap_peer ()
{
	paper::endpoint result (boost::asio::ip::address_v6::any (), 0);
	std::lock_guard<std::mutex> lock (mutex);
	;
	for (auto i (peers.get<4> ().begin ()), n (peers.get<4> ().end ()); i != n;)
	{
		...
		{
			result = i->endpoint;
			peers.get<4> ().modify (i, [](paper::peer_information & peer_a) {
...
bool paper::parse_port (std::string const & string_a, uint16_t & port_a)
{
	bool result;
	size_t converted;
...
bool paper::parse_address_port (std::string const & string, boost::asio::ip::address & address_a, uint16_t & port_a)
{
	auto result (false);
...
void paper::node::ongoing_keepalive ()
{
	...
	std::weak_ptr<paper::node> node_w (shared_from_this ());
...
void paper::node::ongoing_rep_crawl ()
{
	auto now (std::chrono::steady_clock::now ());
...
void paper::node::ongoing_bootstrap ()
{
...
void paper::node::ongoing_store_flush ()
{
	{
		paper::transaction transaction (store.environment, nullptr, true);
...
{
	assert (balance_a >= amount_a * paper::Gxrb_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= paper::Gxrb_ratio;
		auto balance_scaled ((balance_l / paper::Mxrb_ratio).convert_to<double> ());
...
{
public:
	distributed_work (std::shared_ptr<paper::node> const & node_a, paper::block_hash const & root_a, std::function<void(uint64_t)> callback_a) :
	callback (callback_a),
	node (node_a),
	root (root_a)
	{
		...
		if (!outstanding.empty ())
		{
			...
			for (auto const & i : outstanding)
			{
				...
				node->background ([this_l, host, service]() {
					...
					connection->socket.async_connect (paper::tcp_endpoint (host, service), [this_l, connection](boost::system::error_code const & ec) {
						if (!ec)
						{
	...
	void success (std::string const & body_a, boost::asio::ip::address const & address)
	{
		...
		try
		{
			...
			uint64_t work;
			if (!paper::from_string_hex (work_text, work))
			{
				if (!paper::work_validate (root, work))
				{
					set_once (work);
					stop ();
				}
...
void paper::node::generate_work (paper::block & block_a)
{
	block_a.block_work_set (generate_work (block_a.root ()));
}

void paper::node::generate_work (paper::uint256_union const & hash_a, std::function<void(uint64_t)> callback_a)
{
	auto work_generation (std::make_shared<distributed_work> (shared (), hash_a, callback_a));
	work_generation->start ();
}

uint64_t paper::node::generate_work (paper::uint256_union const & hash_a)
{
...
void paper::node::add_initial_peers ()
{
}
...
{
public:
	confirmed_visitor (paper::node & node_a, std::shared_ptr<paper::block> block_a) :
	node (node_a),
	block (block_a)
	{
	}
	...
	void send_block (paper::send_block const & block_a) override
	{
		for (auto i (node.wallets.items.begin ()), n (node.wallets.items.end ()); i != n; ++i)
		{
			...
			if (wallet->exists (block_a.hashables.destination))
			{
				...
				auto error (node.store.pending_get (transaction, paper::pending_key (block_a.hashables.destination, block_a.hash ()), pending));
				if (!error)
				{
					auto node_l (node.shared ());
					auto amount (pending.amount.number ());
					assert (block.get () == &block_a);
					wallet->receive_async (block, representative, amount, [](std::shared_ptr<paper::block>) {});
				}
				else
				{
					if (node.config.logging.ledger_duplicate_logging ())
...
void paper::node::process_confirmed (std::shared_ptr<paper::block> confirmed_a)
{
...
void paper::block_arrival::add (paper::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	arrival.insert (paper::block_arrival_info{ now, hash_a });
}

bool paper::block_arrival::recent (paper::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
std::unordered_set<paper::endpoint> paper::peer_container::random_set (size_t count_a)
{
	std::unordered_set<paper::endpoint> result;
...
std::vector<paper::peer_information> paper::peer_container::representatives (size_t count_a)
{
...
std::vector<paper::peer_information> paper::peer_container::purge_list (std::chrono::steady_clock::time_point const & cutoff)
{
	std::vector<paper::peer_information> result;
	{
		...
		for (auto i (peers.begin ()), n (peers.end ()); i != n; ++i)
		{
			peers.modify (i, [](paper::peer_information & info) { info.last_attempt = std::chrono::steady_clock::now (); });
		}
...
std::vector<paper::endpoint> paper::peer_container::rep_crawl ()
{
	std::vector<paper::endpoint> result;
	result.reserve (8);
	std::lock_guard<std::mutex> lock (mutex);
...
size_t paper::peer_container::size_sqrt ()
{
...
bool paper::peer_container::rep_response (paper::endpoint const & endpoint_a, paper::amount const & weight_a)
{
	...
	{
		peers.modify (existing, [weight_a, &updated](paper::peer_information & info) {
...
void paper::peer_container::rep_request (paper::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
	if (existing != peers.end ())
	{
		peers.modify (existing, [](paper::peer_information & info) {
			info.last_rep_request = std::chrono::steady_clock::now ();
		});
	}
}

bool paper::peer_container::reachout (paper::endpoint const & endpoint_a)
{
	auto result (false);
...
bool paper::peer_container::insert (paper::endpoint const & endpoint_a, unsigned version_a)
{
	auto unknown (false);
	...
	if (!result)
	{
		...
		{
			peers.insert (paper::peer_information (endpoint_a, version_a));
			unknown = true;
		}
	}
	if (unknown && !result)
...
bool paper::reserved_address (paper::endpoint const & endpoint_a)
{
	...
	else if (bytes.is_loopback () && paper::paper_network != paper::paper_networks::paper_test_network)
	{
		result = true;
	}
	else if (bytes >= ipv4_loopback_min && bytes <= ipv4_loopback_max && paper::paper_network != paper::paper_networks::paper_test_network)
	{
		result = true;
	}
	return result;
...
paper::peer_information::peer_information (paper::endpoint const & endpoint_a, unsigned network_version_a) :
endpoint (endpoint_a),
...
paper::peer_information::peer_information (paper::endpoint const & endpoint_a, std::chrono::steady_clock::time_point const & last_contact_a, std::chrono::steady_clock::time_point const & last_attempt_a) :
endpoint (endpoint_a),
...
void paper::peer_container::contacted (paper::endpoint const & endpoint_a, unsigned version_a)
{
	auto endpoint_l (endpoint_a);
	if (endpoint_l.address ().is_v4 ())
	{
...
paper::election::election (MDB_txn * transaction_a, paper::node & node_a, std::shared_ptr<paper::block> block_a, std::function<void(std::shared_ptr<paper::block>, bool)> const & confirmation_action_a) :
...
void paper::election::compute_rep_votes (MDB_txn * transaction_a)
{
	node.wallets.foreach_representative (transaction_a, [this, transaction_a](paper::public_key const & pub_a, paper::raw_key const & prv_a) {
		auto vote (this->node.store.vote_generate (transaction_a, pub_a, prv_a, last_winner));
		this->votes.vote (vote);
	});
}

void paper::election::broadcast_winner ()
{
	{
		paper::transaction transaction (node.store.environment, nullptr, true);
...
paper::uint128_t paper::election::quorum_threshold (MDB_txn * transaction_a, paper::ledger & ledger_a)
{
	// Threshold over which unanimous voting implies confirmation
	return ledger_a.supply (transaction_a) / 2;
}

paper::uint128_t paper::election::minimum_threshold (MDB_txn * transaction_a, paper::ledger & ledger_a)
{
...
void paper::election::confirm_once (MDB_txn * transaction_a)
{
		...
		if (!(*block_l == *last_winner))
		{
			if (exceeded_min_threshold)
			{
				...
				node.background ([node_l, block_l]() {
					node_l->block_processor.process_receive_many (paper::block_processor_item (block_l, true));
				});
				last_winner = block_l;
			}
			else
...
bool paper::election::have_quorum (MDB_txn * transaction_a)
{
	auto tally_l (node.ledger.tally (transaction_a, votes));
	assert (tally_l.size () > 0);
...
void paper::election::confirm_if_quorum (MDB_txn * transaction_a)
{
...
void paper::election::confirm_cutoff (MDB_txn * transaction_a)
{
	if (node.config.logging.vote_logging ())
	{
...
void paper::election::vote (std::shared_ptr<paper::vote> vote_a)
{
	node.network.republish_vote (last_vote, vote_a);
	last_vote = std::chrono::steady_clock::now ();
	paper::transaction transaction (node.store.environment, nullptr, true);
	assert (node.store.vote_validate (transaction, vote_a).code != paper::vote_code::invalid);
	votes.vote (vote_a);
	confirm_if_quorum (transaction);
}

void paper::active_transactions::announce_votes ()
{
	std::vector<paper::block_hash> inactive;
	paper::transaction transaction (node.store.environment, nullptr, true);
	std::lock_guard<std::mutex> lock (mutex);

	{
		...
		for (; i != n && announcements < announcements_per_interval; ++i)
		{
			...
			else
			{
				...
				roots.modify (i, [&announcements](paper::conflict_info & info_a) {
		...
		for (; i != n; ++i)
		{
			...
			roots.modify (i, [](paper::conflict_info & info_a) {
...
void paper::active_transactions::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	roots.clear ();
}

bool paper::active_transactions::start (MDB_txn * transaction_a, std::shared_ptr<paper::block> block_a, std::function<void(std::shared_ptr<paper::block>, bool)> const & confirmation_action_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto root (block_a->root ());
	...
	{
		auto election (std::make_shared<paper::election> (transaction_a, node, block_a, confirmation_action_a));
		roots.insert (paper::conflict_info{ root, election, 0 });
	}
	return existing != roots.end ();
}

// Validate a vote and apply it to the current election if one exists
void paper::active_transactions::vote (std::shared_ptr<paper::vote> vote_a)
{
...
bool paper::active_transactions::active (paper::block const & block_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	return roots.find (block_a.root ()) != roots.end ();
}

paper::active_transactions::active_transactions (paper::node & node_a) :
node (node_a)
{
}

int paper::node::store_version ()
{
	paper::transaction transaction (store.environment, nullptr, false);
	return store.version_get (transaction);
}

paper::thread_runner::thread_runner (boost::asio::io_service & service_a, unsigned service_threads_a)
{
...
paper::thread_runner::~thread_runner ()
{
	join ();
}
...
void paper::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
bool paper::handle_node_options (boost::program_options::variables_map & vm)
{
	auto result (false);
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : paper::working_path ();
	...
	else if (vm.count ("diagnostics"))
	{
		...
		paper::raw_key key;
		key.data.clear ();
		paper::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		paper::raw_key junk1;
		junk1.data.clear ();
		paper::uint256_union junk2 (0);
		paper::kdf kdf;
		kdf.phs (junk1, "", junk2);
		std::cout << "Dumping OpenCL information" << std::endl;
		bool error (false);
		paper::opencl_environment environment (error);
		if (!error)
		{
		...
		if (vm.count ("wallet") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (existing != node.node->wallets.items.end ())
				{
					...
					{
						paper::transaction transaction (existing->second->store.environment, nullptr, false);
						paper::raw_key seed;
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node.node->wallets.items.begin ()), n (node.node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << paper::uint256_union (j->first.uint256 ()).to_account () << '\n';
			}
		}
	}
		...
		if (vm.count ("wallet") == 1 && vm.count ("account") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != node.node->wallets.items.end ())
				{
					...
					{
						paper::transaction transaction (wallet->second->store.environment, nullptr, true);
	...
	else if (vm.count ("wallet_representative_get"))
	{
		if (vm.count ("wallet") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != node.node->wallets.items.end ())
				{
					paper::transaction transaction (wallet->second->store.environment, nullptr, false);
	...
	{
		inactive_node node (data_path);
		paper::transaction transaction (node.node->store.environment, nullptr, false);
		for (auto i (node.node->store.vote_begin (transaction)), n (node.node->store.vote_end ()); i != n; ++i)
		{
			...
			auto vote (std::make_shared<paper::vote> (error, stream));
			assert (!error);
...
paper::inactive_node::inactive_node (boost::filesystem::path const & path) :
path (path),
...
work (1, nullptr)
{
	...
	logging.init (path);
	node = std::make_shared<paper::node> (init, *service, 24000, path, alarm, logging, work);
}

paper::inactive_node::~inactive_node ()
{
	node->stop ();
}

paper::port_mapping::port_mapping (paper::node & node_a) :
node (node_a),
devices (nullptr),
...
void paper::port_mapping::start ()
{
	check_mapping_loop ();
}

void paper::port_mapping::refresh_devices ()
{
	if (paper::paper_network != paper::paper_networks::paper_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void paper::port_mapping::refresh_mapping ()
{
	if (paper::paper_network != paper::paper_networks::paper_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
...
int paper::port_mapping::check_mapping ()
{
	int result (3600);
	if (paper::paper_network != paper::paper_networks::paper_test_network)
	{
...
void paper::port_mapping::check_mapping_loop ()
{
