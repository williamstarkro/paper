...
{
	bool error (false);
	paper::open_or_create (stream_a, path_a.string ());
	if (!stream_a.fail ())
	{
		boost::property_tree::ptree tree;
		try
...
public:
	...
	mdb_val (size_t, void *);
	mdb_val (paper::uint128_union const &);
	mdb_val (paper::uint256_union const &);
	void * data () const;
	size_t size () const;
	paper::uint256_union uint256 () const;
...
/**
 * PPRI wrapper of MDB_txn where the constructor starts the transaction
