...
work (1, nullptr)
{
	...
	for (size_t i (0); i < count_a; ++i)
	{
		...
		auto node (std::make_shared<paper::node> (init, service, paper::unique_path (), alarm, config, work));
		assert (!init.error ());
		node->start ();
...
void paper::system::generate_rollback (paper::node & node_a, std::vector<paper::account> & accounts_a)
{
	paper::block_hash current (node_a.latest (get_random_account (accounts_a)));
	paper::block_hash target (current);
	paper::transaction transaction (node_a.store.environment, nullptr, true);
	while (!current.is_zero ())
	{
		...
		auto open (dynamic_cast<paper::open_block *> (block2.get ()));
		if (open != nullptr)
		{
...
void paper::system::generate_receive (paper::node & node_a)
{
	std::shared_ptr<paper::send_block> send_block;
	{
		paper::transaction transaction (node_a.store.environment, nullptr, false);
		paper::uint256_union random_block;
		random_pool.GenerateBlock (random_block.bytes.data (), sizeof (random_block.bytes));
		auto i (node_a.store.pending_begin (transaction, paper::pending_key (random_block, 0)));
		if (i != node_a.store.pending_end ())
		{
			paper::pending_key send_hash (i->first);
			...
			send_block.reset (static_cast<paper::send_block *> (block.release ()));
		}
	}
	if (send_block != nullptr)
	{
		auto receive_error (wallet (0)->receive_sync (std::move (send_block), paper::genesis_account, std::numeric_limits<paper::uint128_t>::max ()));
		(void)receive_error;
	}
}

void paper::system::generate_activity (paper::node & node_a, std::vector<paper::account> & accounts_a)
{
...
paper::account paper::system::get_random_account (std::vector<paper::account> & accounts_a)
{
...
void paper::system::generate_send_existing (paper::node & node_a, std::vector<paper::account> & accounts_a)
{
	...
	paper::account source;
	{
		...
		destination = paper::account (entry->first.uint256 ());
...
void paper::system::generate_change_known (paper::node & node_a, std::vector<paper::account> & accounts_a)
{
	paper::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		paper::account destination (get_random_account (accounts_a));
		auto change_error (wallet (0)->change_sync (source, destination));
		assert (!change_error);
	}
}

void paper::system::generate_change_unknown (paper::node & node_a, std::vector<paper::account> & accounts_a)
{
	paper::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		paper::keypair key;
		paper::account destination (key.pub);
		auto change_error (wallet (0)->change_sync (source, destination));
		assert (!change_error);
	}
}

void paper::system::generate_send_new (paper::node & node_a, std::vector<paper::account> & accounts_a)
{
...
{
	std::vector<paper::account> accounts;
	wallet (0)->insert_adhoc (paper::test_genesis_key.prv);
