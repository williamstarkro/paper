...
{
	paper::publish publish (std::unique_ptr<paper::block> (new paper::send_block (0, 1, 2, paper::keypair ().prv, 4, 5)));
...
{
	paper::keypair key1;
	auto vote (std::make_shared<paper::vote> (key1.pub, key1.prv, 0, std::unique_ptr<paper::block> (new paper::send_block (0, 1, 2, key1.prv, 4, 5))));
