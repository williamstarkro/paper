...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % PAPER_VERSION_MAJOR % PAPER_VERSION_MINOR).c_str ());
...
wallet (wallet_a)
{
	...
	QObject::connect (create_account, &QPushButton::released, [this]() {
		paper::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, true);
	...
	QObject::connect (backup_seed, &QPushButton::released, [this]() {
		paper::raw_key seed;
		paper::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
		if (this->wallet.wallet_m->store.valid_password (transaction))
		{
...
void paper_qt::accounts::refresh_wallet_balance ()
{
	paper::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
...
paper_qt::history::history (paper::ledger & ledger_a, paper::account const & account_a, paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
{
public:
	short_text_visitor (MDB_txn * transaction_a, paper::ledger & ledger_a) :
	...
	void open_block (paper::open_block const & block_a)
	{
		...
		{
			account = paper::genesis_account;
			amount = paper::genesis_amount;
		}
	}
...
wallet (wallet_a)
{
	...
	QObject::connect (retrieve, &QPushButton::released, [this]() {
		...
		{
			paper::transaction transaction (this->wallet.node.store.environment, nullptr, false);
...
paper_qt::account_viewer::account_viewer (paper_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
std::string paper_qt::status::text ()
{
	...
	{
		paper::transaction transaction (wallet.wallet_m->node.store.environment, nullptr, false);
	...
	switch (*active.begin ())
	{
		case paper_qt::status_types::disconnected:
			...
			break;
		case paper_qt::status_types::working:
		...
		case paper_qt::status_types::locked:
			...
			break;
		case paper_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case paper_qt::status_types::active:
...
std::string paper_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
paper_qt::wallet::wallet (QApplication & application_a, paper_qt::eventloop_processor & processor_a, paper::node & node_a, std::shared_ptr<paper::wallet> wallet_a, paper::account & account_a) :
rendering_ratio (paper::Mxrb_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
active_status (*this)
{
	...
	send_account->setPlaceholderText (paper::zero_key.pub.to_account ().c_str ());
...
void paper_qt::wallet::start ()
{
	std::weak_ptr<paper_qt::wallet> this_w (shared_from_this ());
			...
			if (!amount.decode_dec (this_l->send_count->text ().toStdString ()))
			{
				...
				if (actual / this_l->rendering_ratio == amount.number ())
				{
					...
					if (!parse_error)
					{
						...
						if (actual <= balance)
						{
							paper::transaction transaction (this_l->wallet_m->store.environment, nullptr, false);
							if (this_l->wallet_m->store.valid_password (transaction))
							{
								...
								this_l->node.background ([this_w, account_l, actual]() {
									if (auto this_l = this_w.lock ())
									{
										this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<paper::block> block_a) {
											if (auto this_l = this_w.lock ())
											{
	...
	node.observers.account_balance.add ([this_w](paper::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (paper_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (paper_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](paper::endpoint const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (paper_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (paper_qt::status_types::synchronizing);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (paper_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (paper_qt::status_types::working);
					}
				}
			}));
		}
...
void paper_qt::wallet::empty_password ()
{
...
void paper_qt::wallet::change_rendering_ratio (paper::uint128_t const & rendering_ratio_a)
{
...
std::string paper_qt::wallet::format_balance (paper::uint128_t const & balance) const
{
	...
	else if (rendering_ratio == paper::xrb_ratio)
	{
		unit = std::string ("xrb");
	}
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (paper::zero_key.pub.to_account ().c_str ());
	...
	QObject::connect (change_rep, &QPushButton::released, [this]() {
		...
		if (!representative_l.decode_account (new_representative->text ().toStdString ()))
		{
			...
			if (this->wallet.wallet_m->store.valid_password (transaction))
			{
				...
				{
					paper::transaction transaction_l (this->wallet.wallet_m->store.environment, nullptr, true);
...
void paper_qt::settings::refresh_representative ()
{
	paper::transaction transaction (this->wallet.wallet_m->node.store.environment, nullptr, false);
...
void paper_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (paper_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (paper_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (paper_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (paper_qt::status_types::vulnerable);
	}
}
...
mpaper (new QRadioButton ("Mxrb")),
kpaper (new QRadioButton ("kxrb")),
paper (new QRadioButton ("xrb")),
back (new QPushButton ("Back")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mpaper);
	ratio_group->addButton (kpaper);
	ratio_group->addButton (paper);
	ratio_group->setId (mpaper, 0);
	ratio_group->setId (kpaper, 1);
	ratio_group->setId (paper, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mpaper);
	scale_layout->addWidget (kpaper);
	scale_layout->addWidget (paper);
	...
	QObject::connect (mpaper, &QRadioButton::toggled, [this]() {
		if (mpaper->isChecked ())
		{
			this->wallet.change_rendering_ratio (paper::Mxrb_ratio);
		}
	});
	QObject::connect (kpaper, &QRadioButton::toggled, [this]() {
		if (kpaper->isChecked ())
		{
			this->wallet.change_rendering_ratio (paper::kxrb_ratio);
		}
	});
	QObject::connect (paper, &QRadioButton::toggled, [this]() {
		if (paper->isChecked ())
		{
			this->wallet.change_rendering_ratio (paper::xrb_ratio);
		}
	});
	mpaper->click ();
...
void paper_qt::advanced_actions::refresh_ledger ()
{
	...
	{
		QList<QStandardItem *> items;
		items.push_back (new QStandardItem (QString (paper::block_hash (i->first.uint256 ()).to_account ().c_str ())));
		paper::account_info info (i->second);
		std::string balance;
		paper::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void paper_qt::block_creation::create_receive ()
{
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			auto send_block (dynamic_cast<paper::send_block *> (block_l.get ()));
			if (send_block != nullptr)
			{
				paper::pending_key pending_key (send_block->hashables.destination, source_l);
				...
				if (!wallet.node.store.pending_get (transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							paper::receive_block receive (info.head, source_l, key, pending_key.account, wallet.wallet_m->work_fetch (transaction, pending_key.account, info.head));
							std::string block_l;
				...
				if (send_block != nullptr)
				{
					...
					if (!wallet.node.store.pending_get (transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								paper::open_block open (source_l, representative_l, pending_key.account, key, pending_key.account, wallet.wallet_m->work_fetch (transaction, pending_key.account, pending_key.account));
								std::string block_l;
