...
paper_qt::client::client (QApplication & application_a, paper::client & client_a) :
client_m (client_a),
...
wallet_add_account (new QPushButton ("Create account"))
{
    ...
    QObject::connect (settings_bootstrap_button, &QPushButton::released, [this] ()
    {
        ...
        if (!paper::parse_tcp_endpoint (address_text, endpoint))
        {
            QPalette palette;
    ...
    QObject::connect (settings_connect_button, &QPushButton::released, [this] ()
    {
        ...
        if (!paper::parse_endpoint (address_text, endpoint))
        {
            QPalette palette;
            palette.setColor (QPalette::Text, Qt::black);
            settings_connect_line->setPalette (palette);
    ...
    QObject::connect (send_blocks_send, &QPushButton::released, [this] ()
    {
        ...
        try
        {
            ...
            paper::uint128_t coins (advanced.scale_up (scaled));
...
void paper_qt::client::refresh_wallet ()
{
    paper::uint128_t balance;
...
paper_qt::client::~client ()
{
}

void paper_qt::client::push_main_stack (QWidget * widget_a)
{
    main_stack->addWidget (widget_a);
    main_stack->setCurrentIndex (main_stack->count () - 1);
}

void paper_qt::client::pop_main_stack ()
{
    main_stack->removeWidget (main_stack->currentWidget ());
}

void paper_qt::password_change::clear ()
{
...
paper_qt::password_change::password_change (paper_qt::client & client_a) :
window (new QWidget),
...
paper_qt::enter_password::enter_password (paper_qt::client & client_a) :
window (new QWidget),
layout (new QVBoxLayout),
valid (new QLabel),
password (new QLineEdit),
...
void paper_qt::enter_password::activate ()
{
    client.push_main_stack (window);
    update_label ();
}

void paper_qt::enter_password::update_label ()
{
    if (client.client_m.wallet.valid_password ())
    {
...
paper_qt::advanced_actions::advanced_actions (paper_qt::client & client_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void paper_qt::advanced_actions::refresh_log ()
{
    QStringList log;
...
uint64_t paper_qt::advanced_actions::scale_down (paper::uint128_t const & amount_a)
{
    return (amount_a / scale).convert_to <uint64_t> ();
}

paper::uint128_t paper_qt::advanced_actions::scale_up (uint64_t amount_a)
{
