...
pub fn launch_node(
    ...
    let data_dir = tmp_dir.join(format!("Paper_load_test_{}", i));
    match fs::create_dir(&data_dir) {
        Ok(_) => {}
        Err(ref e) if e.kind() == io::ErrorKind::AlreadyExists => {
            let _ = fs::remove_file(data_dir.join("data.ldb"));
        }
        r => r.chain_err(|| "failed to create paper_node data directory")?,
    }
    ...
    let child = Command::new(paper_node)
        .arg("--data_path")
        .arg(&data_dir)
        .arg("--daemon")
        .spawn_async(&handle)
        .chain_err(|| "failed to spawn paper_node")?;
...
pub fn connect_node<C: Connect>(
    ...
    })).then(|x| x.chain_err(|| "failed to call paper_node RPC"))
            .map(|_| ()),
    ) as _
}
